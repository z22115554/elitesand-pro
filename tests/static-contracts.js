...
const UNPROTECTED_ROUTE_ALLOWLIST = new Map([
  ...
  ['server/routes/api.js:POST:/webgpu-separation/result/:jobId', 'called only by the hidden Electron webgpu-engine window, which has no PIN context — same exemption rationale as the webgpu-engine socket clientType in PIN_EXEMPT_CLIENT_TYPES; authorized instead by requireLocalEngineJob (loopback-only + random jobId matching the current activeJob), which must stay ahead of the multer upload'],
]);
