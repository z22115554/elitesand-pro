...
test('renderer keeps update checking manual and only exposes restart after a positive result', () => {
  ...
  assert.ok(!index.includes('app-update-check.js'));
  assert.ok(index.includes('app-update-check-btn'));
  assert.ok(index.includes('app-version-current'));
  assert.ok(index.includes('app-update-status'));
  assert.ok(index.includes('app-update-restart-btn'));
  assert.ok(!toast.includes('/api/app-update/') && panel.includes("/api/update-check?force=1"));
  assert.ok(!panel.includes('/api/app-update/') && !panel.includes('/api/app-update/apply'));
