...
test('Twitch 點歌規則契約驗證自訂指令、別名與聊天室徽章權限', () => {
  const settings = {
    ...TwitchRequestSettings.getDefaults(),
    command: '!唱歌', aliases: ['!song', '!來一首'], permissionLevel: 'subscriber',
  };
  ...
  eq(TwitchRequestSettings.validateSettings({ ...settings, aliases: ['!唱歌'] }).ok, false);
  eq(TwitchRequestSettings.validateSettings({ ...settings, command: '唱歌' }).ok, false);
});
...
test('Twitch 回覆與點歌規則 socket 只接受 controller，並由 server 再驗證', () => {
  ...
  eq(ctx.playState.twitchRequestSettings.command, '!唱歌');
...
test('Twitch 點歌頁有指令規則、回覆測試、變數驗證與各自還原入口', () => {
  ...
  ok(html.includes('id="twitch-request-command"') && html.includes('id="twitch-request-aliases"'));
  ok(html.includes('id="twitch-request-permission"') && html.includes('id="twitch-request-cooldown"'));
