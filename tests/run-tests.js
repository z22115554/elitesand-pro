...
test('播放音量在 SoundTouch 先啟動時仍會寫入記憶，KTV 掃色對齊逐字來源且走合成器', () => {
  ...
  ok(ktv.includes('COUNTDOWN_PATTERNS') && ktv.includes("['★', '●', '★', '●', '★']"), 'KTV 間奏倒數必須保留混合星星與圓點版本：');
  ok(ktv.includes('if (timeMs < countdownStart) showFiller') && ktv.includes('else clearSlot(curSlot);'), 'KTV 間奏文案必須在倒數開始時清除，不能與倒數同步掃色：');
});
...
test('KTV 掃色以 Facet 相同的下一字起點接續，字間空拍不會停住', () => {
  ...
  eq(interludeTopFillWindow.style.transform, 'scaleX(1)', '間奏文字必須立刻完整填色，不得與倒數共用掃色：');
  ...
  ok(countdownText.includes('★') && countdownText.includes('●'), '倒數必須同時帶有星星與圓點：');
