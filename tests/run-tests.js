...
test('播放音量在 SoundTouch 先啟動時仍會寫入記憶，KTV 掃色對齊逐字來源且走合成器', () => {
  ...
  ok(ktv.includes('if (timeMs >= countdownStart) clearSlot(curSlot);'), 'KTV 間奏文案必須在倒數開始時清除：');
...
test('KTV 掃色以 Facet 相同的下一字起點接續，字間空拍不會停住', () => {
  ...
  const countdownBottom = container.children[0].children[1];
  const countdownText = countdownBottom.children[0].textContent;
  eq(interludeTop.children[0].textContent, '', '倒數開始後必須先清除間奏文字：');
  ok(['★ ★ ★ ★ ★', '● ● ● ● ●'].includes(countdownText), '倒數必須是五個同圖案，不得混合星星與圓點：');
  eq(countdownBottom.children[1].style.transform, 'scaleX(0)', '倒數開始時必須從零進度起跑：');
  template.onFrame(7500, ctx);
  eq(countdownBottom.children[1].style.transform, 'scaleX(0.1)', '下一句前五秒才開始掃倒數圖案：');
  template.onFrame(7450, ctx);
  eq(countdownBottom.children[1].style.transform, 'scaleX(0.1)', '正常播放的微小回撥不可讓倒數圖案回彈：');
  template.onSeek(7450, ctx);
  eq(countdownBottom.children[1].style.transform, 'scaleX(0.09)', '明確 seek 必須仍可讓倒數回跳到正確位置：');
  template.onFrame(12000, ctx);
  eq(countdownBottom.children[0].textContent, '乙', '下一句起點必須立即清掉倒數並讓位給歌詞：');
  eq(countdownBottom.children[1].style.transform, 'scaleX(0)', '下一句起點必須從尚未掃色的歌詞開始：');
  ...
  eq(container.children[0].children[0].children[0].textContent, '', '短間奏也必須在倒數開始時清掉上一句：');
  ok(
    ['★ ★ ★ ★ ★', '● ● ● ● ●'].includes(container.children[0].children[1].children[0].textContent),
    '短間奏必須直接交給五個同圖案倒數：',
  );
});
