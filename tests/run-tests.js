...
test('播放音量在 SoundTouch 先啟動時仍會寫入記憶，KTV 掃色對齊逐字來源且走合成器', () => {
  ...
  ok(ktv.includes('FILLER_SWEEP_MS') && ktv.includes('if (timeMs < countdownStart) showFiller') && ktv.includes('else clearSlot(curSlot);'), 'KTV 間奏文案必須獨立掃色並在倒數開始時清除：');
...
test('KTV 掃色以 Facet 相同的下一字起點接續，字間空拍不會停住', () => {
  ...
  eq(interludeTopFillWindow.style.transform, 'scaleX(0.45454545454545453)', '間奏文字必須先以自己的時間軸掃色：');
  template.onFrame(2500, ctx);
  eq(interludeTopFillWindow.style.transform, 'scaleX(1)', '間奏文字掃完後必須停留完整內容，不得與倒數共用掃色：');

  template.onFrame(4500, ctx);
