...
test('v0.9.2 強制完整升級公告覆蓋舊版並在新版收起', () => {
  const document = announcementService.validateDocument(JSON.parse(
    fs.readFileSync(path.join(__dirname, '..', 'announcement.json'), 'utf8'),
  ));
  eq(document.announcements.length, 1);
  const notice = document.announcements[0];
  eq(notice.id, 'v0.9.2-required-full-update');
  ...
  ok(announcementService.versionMatches(notice, '0.9.0'));
  ok(announcementService.versionMatches(notice, '0.9.1'));
