/*
 * SoundTouch AudioWorklet processor — 即時變調/變速（跑在音訊執行緒）
 *
 * 演算法核心(FifoSampleBuffer / RateTransposer / Stretch / SoundTouch / SimpleFilter) 取自
 * SoundTouch JS v0.3.0 (LGPL) — Copyright (c) Olli Parviainen / Ryan Berdeen / Jakub Fiala /
 * Steve 'Cutter' Blades。原 vendor/soundtouch.js 是 ScriptProcessor(主執行緒) 版；本檔把同一套
 * 演算法包進 AudioWorkletProcessor，改在「音訊執行緒」即時拉取，因此：
 *   1) 不受主執行緒(GSAP 歌詞動畫/粒子) 卡頓影響 → 無 buffer underrun 爆音；
 *   2) 改 pitch/tempo 只是 postMessage，即時生效 → 零重算等待。
 *
 * 整首音檔的 channel data 於建立時由主執行緒一次 transfer 進來，之後播放/暫停/seek/變調皆走 port 訊息。
 * 對外位置(position) 一律以「原曲 sample 數」回報(SimpleFilter.sourcePosition 即原曲讀取位置)，
 * 故變速時時間軸仍對齊原曲。
 */

/* eslint-disable */
class FifoSampleBuffer {
  constructor() { this._vector = new Float32Array(); this._position = 0; this._frameCount = 0; }
  ...
  receive(numFrames) {
    ...
    this._frameCount -= numFrames; this._position += numFrames;
  }
  receiveSamples(output, numFrames = 0) {
    const numSamples = numFrames * 2; const sourceOffset = this.startIndex;
    output.set(this._vector.subarray(sourceOffset, sourceOffset + numSamples));
    this.receive(numFrames);
  }
  extract(output, position = 0, numFrames = 0) {
    const sourceOffset = this.startIndex + position * 2; const numSamples = numFrames * 2;
    output.set(this._vector.subarray(sourceOffset, sourceOffset + numSamples));
  }
  ensureCapacity(numFrames = 0) {
    ...
    if (this._vector.length < minLength) {
      ...
      this._vector = newVector; this._position = 0;
    } else { this.rewind(); }
  }
  ensureAdditionalCapacity(numFrames = 0) { this.ensureCapacity(this._frameCount + numFrames); }
  rewind() {
    if (this._position > 0) {
      this._vector.set(this._vector.subarray(this.startIndex, this.endIndex)); this._position = 0;
    }
  }
}
...
class AbstractFifoSamplePipe {
  constructor(createBuffers) {
    if (createBuffers) { this._inputBuffer = new FifoSampleBuffer(); this._outputBuffer = new FifoSampleBuffer(); }
...
class RateTransposer extends AbstractFifoSamplePipe {
  constructor(createBuffers) { super(createBuffers); this.reset(); this._rate = 1; }
  ...
  process() {
    ...
    this._inputBuffer.receive(); this._outputBuffer.put(numFramesOutput);
  }
  transpose(numFrames = 0) {
    if (numFrames === 0) { return 0; }
    const src = this._inputBuffer.vector; const srcOffset = this._inputBuffer.startIndex;
    const dest = this._outputBuffer.vector; const destOffset = this._outputBuffer.endIndex;
    let used = 0; let i = 0;
    while (this.slopeCount < 1.0) {
      dest[destOffset + 2 * i] = (1.0 - this.slopeCount) * this.prevSampleL + this.slopeCount * src[srcOffset];
      dest[destOffset + 2 * i + 1] = (1.0 - this.slopeCount) * this.prevSampleR + this.slopeCount * src[srcOffset + 1];
      i = i + 1; this.slopeCount += this._rate;
    }
    this.slopeCount -= 1.0;
    if (numFrames !== 1) {
      out: while (true) {
        while (this.slopeCount > 1.0) {
          this.slopeCount -= 1.0; used = used + 1;
...
class FilterSupport {
  ...
  fillInputBuffer() { throw new Error('fillInputBuffer() not overridden'); }
...
class SimpleFilter extends FilterSupport {
  constructor(sourceSound, pipe, callback = noop) {
    super(pipe);
    this.callback = callback; this.sourceSound = sourceSound;
    this.historyBufferSize = 22050; this._sourcePosition = 0;
    this.outputBufferPosition = 0; this._position = 0;
  }
  get position() { return this._position; }
  get sourcePosition() { return this._sourcePosition; }
  set sourcePosition(sourcePosition) { this.clear(); this._sourcePosition = sourcePosition; }
  onEnd() { this.callback(); }
  ...
  clear() { super.clear(); this.outputBufferPosition = 0; }
}
...
class Stretch extends AbstractFifoSamplePipe {
  constructor(createBuffers) {
    super(createBuffers);
    this._quickSeek = true; this.midBufferDirty = false; this.midBuffer = null;
    this.overlapLength = 0; this.autoSeqSetting = true; this.autoSeekSetting = true; this._tempo = 1;
    this.setParameters(44100, DEFAULT_SEQUENCE_MS, DEFAULT_SEEKWINDOW_MS, DEFAULT_OVERLAP_MS);
  }
  clear() { super.clear(); this.clearMidBuffer(); }
  clearMidBuffer() {
    this.midBufferDirty = false; this.midBuffer = null;
  ...
  seekBestOverlapPositionStereo() {
    let bestOffset; let bestCorrelation; let correlation; let i = 0;
    this.preCalculateCorrelationReferenceStereo();
    bestOffset = 0; bestCorrelation = Number.MIN_VALUE;
  ...
  seekBestOverlapPositionStereoQuick() {
    let bestOffset; let bestCorrelation; let correlation; let scanCount = 0;
    let correlationOffset; let tempOffset;
    this.preCalculateCorrelationReferenceStereo();
    bestCorrelation = Number.MIN_VALUE; bestOffset = 0; correlationOffset = 0; tempOffset = 0;
  ...
  preCalculateCorrelationReferenceStereo() {
    let i = 0; let context; let temp;
    for (; i < this.overlapLength; i = i + 1) {
      temp = i * (this.overlapLength - i); context = i * 2;
  ...
  calculateCrossCorrelationStereo(mixingPosition, compare) {
    ...
    let correlation = 0; let i = 2; const calcLength = 2 * this.overlapLength; let mixingOffset;
  ...
  overlapStereo(inputPosition) {
    ...
    let i = 0; let context; let tempFrame; const frameScale = 1 / this.overlapLength;
    let fi; let inputOffset; let outputOffset;
    for (; i < this.overlapLength; i = i + 1) {
      tempFrame = (this.overlapLength - i) * frameScale; fi = i * frameScale; context = 2 * i;
      inputOffset = context + inputPosition; outputOffset = context + outputPosition;
      output[outputOffset + 0] = input[inputOffset + 0] * fi + this.midBuffer[context + 0] * tempFrame;
      output[outputOffset + 1] = input[inputOffset + 1] * fi + this.midBuffer[context + 1] * tempFrame;
    }
  }
  process() {
    let offset; let temp; let overlapSkip;
...
class SoundTouch {
  ...
  set pitchOctaves(pitchOctaves) { this.pitch = Math.exp(0.69314718056 * pitchOctaves); this.calculateEffectiveRateAndTempo(); }
  set pitchSemitones(pitchSemitones) { this.pitchOctaves = pitchSemitones / 12.0; }
  get inputBuffer() { return this._inputBuffer; }
  get outputBuffer() { return this._outputBuffer; }
  calculateEffectiveRateAndTempo() {
    const previousTempo = this._tempo; const previousRate = this._rate;
...
/** 從主執行緒 transfer 進來的整首 channel data 當音源（取代讀 AudioBuffer 的 WebAudioBufferSource）。 */
class ArraySource {
  constructor(left, right) { this.left = left; this.right = right; this.length = left.length; }
  extract(target, numFrames = 0, position = 0) {
    const left = this.left, right = this.right, len = this.length;
    for (let i = 0; i < numFrames; i++) {
      const p = i + position;
      if (p < len) { target[i * 2] = left[p]; target[i * 2 + 1] = right[p]; }
      else { target[i * 2] = 0; target[i * 2 + 1] = 0; }
    }
    return Math.min(numFrames, len - position);
  }
}

const POSITION_REPORT_FRAMES = 1024; // 約 23ms@44.1k 回報一次播放位置
...
class SoundTouchProcessor extends AudioWorkletProcessor {
  ...
  _onMessage(msg) {
    switch (msg.type) {
      case 'load': {
        ...
        this._tempo = msg.tempo || 1;
        this._st.pitchSemitones = this._pitch;
        this._st.tempo = this._tempo;
        this._filter = new SimpleFilter(src, this._st);
        this._ready = true;
        this._ended = false;
        this._playing = false;
        break;
      ...
      case 'pause':
        ...
        break;
      case 'seek':
        if (this._ready && typeof msg.position === 'number') {
          this._filter.sourcePosition = Math.max(0, msg.position);
          this._ended = false;
        }
        break;
      case 'pitch':
        this._pitch = msg.value;
        if (this._st) this._st.pitchSemitones = msg.value; // 即時生效，無重算
        break;
      case 'tempo':
        this._tempo = msg.value;
  ...
  process(inputs, outputs) {
    ...
    const extracted = this._filter.extract(this._interleaved, frames);
    let i = 0;
    for (; i < extracted; i++) { left[i] = this._interleaved[i * 2]; right[i] = this._interleaved[i * 2 + 1]; }
    for (; i < frames; i++) { left[i] = 0; right[i] = 0; }

    this._posCounter += frames;
    if (this._posCounter >= POSITION_REPORT_FRAMES) { this._posCounter = 0; this._postPosition(); }

    if (extracted === 0 && !this._ended) {
      this._ended = true;
      this._playing = false;
      this._postPosition();
