...
(function (root, factory) {
  ...
  const ROWS = {
    ...
    'spout.runningHint': ['Shoost 請選擇「{name}」。停止時會先送出透明畫面以清除殘留。', 'In Shoost, select “{name}”. Stopping sends a transparent frame first to clear any residue.', 'Shoost で「{name}」を選択してください。停止時は残像を消すため、先に透明フレームを送信します。', 'Shoost에서 “{name}”을 선택하세요. 중지할 때 잔상을 지우기 위해 먼저 투명 프레임을 보냅니다.', '请在 Shoost 中选择“{name}”。停止时会先发送透明画面以清除残留。'],
    ...
    'home.tab.obs': ['OBS 輸出', 'OBS output', 'OBS 出力', 'OBS 출력', 'OBS 输出'],
    ...
    'import.separation.processing': ['已匯入，人聲分離中 {percent}', 'Imported; separating vocals {percent}', 'インポート完了。ボーカル分離中 {percent}', '가져오기 완료. 보컬 분리 중 {percent}', '已导入，人声分离中 {percent}'],
    ...
    'aiInstall.stagePython': ['安裝 Python 主引擎…', 'Installing Python primary engine…', 'Python メインエンジンをインストール中…', 'Python 기본 엔진 설치 중…', '正在安装 Python 主引擎…'],
    ...
    'particle.effectsLabel': ['描邊 · 陰影', 'Stroke · Shadow', 'ストローク・影', '스트로크 · 그림자', '描边 · 阴影'],
    'particle.strokeWidthLabel': ['描邊寬度', 'Stroke width', 'ストローク幅', '스트로크 두께', '描边宽度'],
    'particle.strokeColorLabel': ['描邊色', 'Stroke color', 'ストローク色', '스트로크 색', '描边色'],
    ...
    'setlist.sceneFontSizeHint': ['場景版的正在播放歌曲文字大小可個別調整；整體大小用場景版設定裡的「整體縮放」。', 'The now-playing text size in scene layouts can be adjusted individually; overall size uses "Overall scale" in the scene layout settings.', 'シーン版の再生中曲のテキストサイズは個別に調整できます。全体のサイズはシーン版設定の「全体スケール」で調整します。', '장면형의 재생 중 곡 텍스트 크기는 개별적으로 조절할 수 있습니다. 전체 크기는 장면형 설정의 "전체 배율"을 사용합니다.', '场景版的正在播放歌曲文字大小可个别调整；整体大小用场景版设置里的“整体缩放”。'],
    ...
    'setlist.sceneScaleLabel': ['整體縮放', 'Overall scale', '全体スケール', '전체 배율', '整体缩放'],
    'setlist.sceneScaleHint': ['把歌單內容整體平移、縮放，避免擋住角色站位；位置/縮放對目前選擇的場景版型各自獨立生效。', 'Shifts and scales the whole setlist content to avoid blocking your character\'s position; position/scale apply independently to the currently selected scene layout.', 'セットリスト全体を移動・拡大縮小し、キャラクターの立ち位置を隠さないようにします。位置／スケールは現在選択中のシーン版レイアウトにのみ独立して適用されます。', '세트리스트 전체를 이동/확대축소해 캐릭터 위치를 가리지 않게 합니다. 위치/배율은 현재 선택된 장면형 레이아웃에만 독립적으로 적용됩니다.', '把歌单内容整体平移、缩放，避免挡住角色站位；位置/缩放对目前选择的场景版型各自独立生效。'],
