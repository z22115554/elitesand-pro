...
(function (root, factory) {
  ...
  const ROWS = {
    ...
    'template.jizuraTitle': ['文字PV：日系文字 PV 風格，依段落自動編排版面與動作，預設放在側欄避開主播', 'Lyric PV: Japanese lyric-video style that plans layouts and motion per song section; sits in a side column by default to keep clear of the streamer', '文字PV：日本の文字PV風に、セクションごとにレイアウトと動きを自動で組み立てます。既定では配信者を避けて横の列に表示します', '텍스트 PV: 일본식 가사 PV 스타일로 섹션별 레이아웃과 동작을 자동 구성하며, 기본값은 스트리머를 피해 측면 열에 표시합니다', '文字PV：日系文字 PV 风格，依段落自动编排版面与动作，默认放在侧栏避开主播'],
    ...
    'jizura.placementRightTitle': ['右側直欄：直式構圖放在畫面右側，避開站在中間的主播', 'Right column: portrait layout on the right side, clear of a streamer standing in the middle', '右の列：縦構図を画面右側に置き、中央に立つ配信者を避けます', '오른쪽 열: 세로 구도를 화면 오른쪽에 두어 가운데 선 스트리머를 피합니다', '右侧直栏：竖式构图放在画面右侧，避开站在中间的主播'],
    'jizura.placementLeftTitle': ['左側直欄：直式構圖放在畫面左側，避開站在中間的主播', 'Left column: portrait layout on the left side, clear of a streamer standing in the middle', '左の列：縦構図を画面左側に置き、中央に立つ配信者を避けます', '왼쪽 열: 세로 구도를 화면 왼쪽에 두어 가운데 선 스트리머를 피합니다', '左侧直栏：竖式构图放在画面左侧，避开站在中间的主播'],
    'jizura.placementFullTitle': ['全畫面：橫式構圖鋪滿整個畫面，歌詞可能壓到主播', 'Full screen: landscape layout across the whole frame; lyrics may cover the streamer', '全画面：横構図で画面全体を使います。歌詞が配信者に重なることがあります', '전체 화면: 가로 구도로 화면 전체를 사용하며 가사가 스트리머를 가릴 수 있습니다', '全画面：横式构图铺满整个画面，歌词可能压到主播'],
    'jizura.placementFull': ['全畫面', 'Full', '全画面', '전체', '全画面'],
    'jizura.placementHint': ['預設右側直欄。全畫面的版面比較有 PV 感，但歌詞常落在畫面正中，會壓到主播，也比較吃效能。', 'Right column by default. Full screen feels more like a PV, but lyrics often land in the middle of the frame, over the streamer, and it costs more performance.', '既定は右の列です。全画面はよりPVらしくなりますが、歌詞が画面中央に来て配信者に重なりがちで、負荷も高くなります。', '기본값은 오른쪽 열입니다. 전체 화면은 더 PV 같지만 가사가 화면 가운데에 와서 스트리머를 가리기 쉽고, 성능 부담도 더 큽니다.', '默认右侧直栏。全画面的版面比较有 PV 感，但歌词常落在画面正中，会压到主播，也比较吃性能。'],
