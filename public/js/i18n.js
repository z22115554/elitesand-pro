...
(function (root, factory) {
  ...
  const ROWS = {
    ...
    'sections.analysisDesc': [
      '自動判斷主歌／副歌／間奏等段落（SongFormer 模型，需要 NVIDIA CUDA 顯卡；第一次使用會下載約 2.7GB 的元件）。',
      'Automatically detects verse/chorus/bridge sections (SongFormer model, needs an NVIDIA CUDA GPU; first use downloads about 2.7GB of components).',
      '主歌・サビ・間奏などのセクションを自動判定します（SongFormer モデル、NVIDIA CUDA GPU が必要。初回利用時に約 2.7GB のコンポーネントをダウンロードします）。',
      '주고/후렴/간주 등 섹션을 자동으로 판단합니다（SongFormer 모델, NVIDIA CUDA GPU 필요; 처음 사용할 때 약 2.7GB 구성 요소를 다운로드합니다）。',
      '自动判断主歌／副歌／间奏等段落（SongFormer 模型，需要 NVIDIA CUDA 显卡；第一次使用会下载约 2.7GB 的元件）。',
    ],
    'sections.stage.load': ['載入模型…', 'Loading the model…', 'モデルを読み込み中…', '모델 불러오는 중…', '载入模型…'],
    'sections.stage.inference': ['分析段落中…', 'Analyzing sections…', 'セクションを分析中…', '섹션 분석 중…', '分析段落中…'],
    'sections.install.note': ['第一次使用要下載約 2.7GB 的元件，只需要一次；下載期間可以繼續用其他功能。', 'First use downloads about 2.7GB of components, only once. You can keep using other features meanwhile.', '初回のみ約 2.7GB のコンポーネントをダウンロードします。その間もほかの機能は使えます。', '처음 한 번만 약 2.7GB 구성 요소를 다운로드합니다. 그동안 다른 기능은 계속 쓸 수 있습니다.', '第一次使用要下载约 2.7GB 的元件，只需要一次；下载期间可以继续用其他功能。'],
    'sections.install.diskCheck': ['檢查磁碟空間…', 'Checking disk space…', 'ディスク容量を確認中…', '디스크 공간 확인 중…', '检查磁盘空间…'],
    'sections.install.downloadPython': ['下載 Python {detail}', 'Downloading Python {detail}', 'Python をダウンロード中 {detail}', 'Python 다운로드 중 {detail}', '下载 Python {detail}'],
    'sections.install.extractPython': ['解壓 Python…', 'Extracting Python…', 'Python を展開中…', 'Python 압축 해제 중…', '解压 Python…'],
    'sections.install.bootstrapPip': ['準備套件安裝工具…', 'Preparing the package installer…', 'パッケージインストーラーを準備中…', '패키지 설치 도구 준비 중…', '准备套件安装工具…'],
    'sections.install.pipResolve': ['確認需要的套件 {name}', 'Resolving packages {name}', '必要なパッケージを確認中 {name}', '필요한 패키지 확인 중 {name}', '确认需要的套件 {name}'],
    'sections.install.pipDownload': ['下載 {name} {detail}', 'Downloading {name} {detail}', '{name} をダウンロード中 {detail}', '{name} 다운로드 중 {detail}', '下载 {name} {detail}'],
    'sections.install.pipInstall': ['安裝 PyTorch 等套件（解壓沒有進度，常需數分鐘，已經過 {elapsed}）', 'Installing PyTorch and other packages (no progress while unpacking, often several minutes; {elapsed} elapsed)', 'PyTorch などをインストール中（展開中は進捗が出ず数分かかることがあります・経過 {elapsed}）', 'PyTorch 등 설치 중(압축 해제 중에는 진행률이 없고 몇 분 걸릴 수 있음, 경과 {elapsed})', '安装 PyTorch 等套件（解压没有进度，常需数分钟，已经过 {elapsed}）'],
    'sections.install.pipInstallSlow': ['安裝 PyTorch 等套件，已經過 {elapsed}——防毒軟體即時掃描時可能要十幾分鐘，不是當機', 'Installing PyTorch and other packages, {elapsed} elapsed — real-time antivirus scanning can stretch this past ten minutes; it has not frozen', 'PyTorch などをインストール中・経過 {elapsed}——ウイルス対策のリアルタイムスキャンで十数分かかることがあります。停止していません', 'PyTorch 등 설치 중, 경과 {elapsed} — 백신 실시간 검사 시 10분 넘게 걸릴 수 있으며 멈춘 것이 아닙니다', '安装 PyTorch 等套件，已经过 {elapsed}——杀毒软件实时扫描时可能要十几分钟，不是死机'],
    'sections.install.downloadSource': ['下載 {name} 原始碼 {detail}', 'Downloading {name} source {detail}', '{name} のソースをダウンロード中 {detail}', '{name} 소스 다운로드 중 {detail}', '下载 {name} 源代码 {detail}'],
    'sections.install.installPython': ['安裝 Python 環境…', 'Installing the Python environment…', 'Python 環境をインストール中…', 'Python 환경 설치 중…', '安装 Python 环境…'],
    'sections.install.installMuq': ['安裝 MuQ（已經過 {elapsed}）', 'Installing MuQ ({elapsed} elapsed)', 'MuQ をインストール中（経過 {elapsed}）', 'MuQ 설치 중(경과 {elapsed})', '安装 MuQ（已经过 {elapsed}）'],
    'sections.install.downloadBackbone': ['下載 MuQ 模型（約 1.3GB）{detail}', 'Downloading the MuQ model (about 1.3GB) {detail}', 'MuQ モデルをダウンロード中（約 1.3GB）{detail}', 'MuQ 모델 다운로드 중(약 1.3GB) {detail}', '下载 MuQ 模型（约 1.3GB）{detail}'],
    'sections.install.downloadWeights': ['下載 SongFormer 權重 {detail}', 'Downloading SongFormer weights {detail}', 'SongFormer の重みをダウンロード中 {detail}', 'SongFormer 가중치 다운로드 중 {detail}', '下载 SongFormer 权重 {detail}'],
    'sections.install.verifyWeights': ['驗證下載的檔案…', 'Verifying downloaded files…', 'ダウンロードしたファイルを検証中…', '다운로드한 파일 확인 중…', '验证下载的文件…'],
    'sections.install.done': ['元件安裝完成，開始分析…', 'Components installed, starting analysis…', 'インストール完了、分析を開始します…', '설치 완료, 분석을 시작합니다…', '元件安装完成，开始分析…'],
    'sections.noCurrentTrack': ['尚未載入歌曲', 'No song loaded yet', 'まだ曲が読み込まれていません', '아직 곡이 로드되지 않았습니다', '尚未载入歌曲'],
    'sections.runButton': ['分析段落', 'Analyze sections', 'セクションを分析', '섹션 분석', '分析段落'],
    'sections.reanalyzeButton': ['重新分析', 'Re-analyze', '再分析', '다시 분석', '重新分析'],
    'sections.preparing': ['準備中…', 'Preparing…', '準備中…', '준비 중…', '准备中…'],
    'sections.currentTrackLabel': ['目前歌曲：{title}', 'Current song: {title}', '現在の曲：{title}', '현재 곡: {title}', '目前歌曲：{title}'],
    'sections.doneCount': ['已分析出 {count} 個段落', 'Found {count} sections', '{count} 個のセクションを検出しました', '{count}개 섹션을 찾았습니다', '已分析出 {count} 个段落'],
    'sections.needsCuda': ['這台電腦偵測不到可用的 NVIDIA CUDA 顯卡，這個功能目前只支援 CUDA。', 'This computer has no usable NVIDIA CUDA GPU — this feature currently only supports CUDA.', 'このパソコンでは利用可能な NVIDIA CUDA GPU が検出できません。この機能は現在 CUDA のみ対応しています。', '이 컴퓨터에서 사용 가능한 NVIDIA CUDA GPU를 감지할 수 없습니다. 이 기능은 현재 CUDA만 지원합니다.', '这台电脑侦测不到可用的 NVIDIA CUDA 显卡，这个功能目前只支持 CUDA。'],
    'sections.gpuBusy': ['GPU 目前被 AI 人聲分離佔用，請等它跑完再試一次。', 'The GPU is currently busy with AI vocal separation — please try again once it finishes.', 'GPU は現在 AI ボーカル分離で使用中です。完了後にもう一度お試しください。', 'GPU가 현재 AI 보컬 분리에 사용 중입니다. 완료 후 다시 시도하세요.', 'GPU 目前被 AI 人声分离占用，请等它跑完再试一次。'],
    'sections.pairingTitle': ['段落搭配', 'Section pairing', 'セクション割り当て', '섹션 매칭', '段落搭配'],
    'sections.pairingDesc': [
      '依段落切換不同的歌詞模板，例如副歌換一個比較有存在感的版面、主歌維持安靜。',
      'Switch to a different lyric template per section — e.g. a bolder layout for the chorus while the verse stays quiet.',
      'セクションごとに異なる歌詞テンプレートに切り替えます（例：サビは存在感のあるレイアウトに、主歌は静かなままに）。',
      '섹션별로 다른 가사 템플릿으로 전환합니다（예: 후렴은 존재감 있는 레이아웃으로, 벌스는 조용하게 유지）.',
      '依段落切换不同的歌词模板，例如副歌换一个比较有存在感的版面、主歌维持安静。',
    ],
    ...
    'sections.testTrackNote': [
      '只對已經用上方「段落分析」分析過的歌曲有作用；沒分析過的歌照常使用目前模板。',
      'Only affects songs that have been analyzed with "Section analysis" above; other songs keep the current template.',
      '上の「セクション分析」で分析済みの曲にのみ適用されます。未分析の曲は現在のテンプレートのままです。',
      '위의 "섹션 분석"으로 분석한 곡에만 적용됩니다. 분석하지 않은 곡은 현재 템플릿을 그대로 사용합니다.',
      '只对已经用上方「段落分析」分析过的歌曲有作用；没分析过的歌照常使用目前模板。',
    ],
    'sections.viewHint': [
    ...
    'nav.moon': ["中秋活動", "Mid-Autumn", "中秋イベント", "추석 이벤트", "中秋活动"],
    'nav.moonTitle': ["中秋活動（限時開放）", "Mid-Autumn event (limited time)", "中秋イベント（期間限定）", "추석 이벤트 (기간 한정)", "中秋活动（限时开放）"],
    'nav.moonLimited': ["（限時開放）", "(Limited time)", "（期間限定）", "(기간 한정)", "（限时开放）"],
    'moon.endNotice': ["限時活動，{date} 結束。時間到了但程式還開著的話可以照常使用；之後重新開啟 Elitesand Pro，這個分頁和 OBS 活動畫面就會自動關閉，斗內紀錄會保留。", "Limited-time event, ends {date}. If Elitesand Pro is still open when time runs out, everything keeps working; the next time you start it, this page and the OBS event overlay close automatically. Donation records are kept.", "期間限定イベントです。{date} に終了します。終了時刻を過ぎてもアプリを開いている間はそのまま使えます。次に Elitesand Pro を起動すると、このページと OBS のイベント画面は自動的に閉じます。投げ銭の記録は残ります。", "기간 한정 이벤트로 {date}에 종료됩니다. 종료 시각이 지나도 프로그램이 열려 있는 동안은 그대로 사용할 수 있으며, 다음에 Elitesand Pro를 다시 실행하면 이 페이지와 OBS 이벤트 화면이 자동으로 닫힙니다. 후원 기록은 보존됩니다.", "限时活动，{date} 结束。时间到了但程序还开着的话可以照常使用；之后重新打开 Elitesand Pro，这个页面和 OBS 活动画面就会自动关闭，打赏记录会保留。"],
    'moon.addTitle': ["新增斗內", "Add donation", "投げ銭を追加", "후원 추가", "添加打赏"],
    'moon.addHint': ["輸入後燈籠會立刻掛上 OBS 畫面，月亮往滿月前進一點。", "The lantern appears on the OBS overlay right away, and the moon grows a little fuller.", "入力するとすぐに OBS 画面に灯籠が掛かり、月が少し満月に近づきます。", "입력하면 바로 OBS 화면에 등불이 걸리고 달이 조금 더 차오릅니다.", "输入后灯笼会立刻挂上 OBS 画面，月亮往满月前进一点。"],
    'moon.name': ["名字", "Name", "名前", "이름", "名字"],
    'moon.amount': ["金額", "Amount", "金額", "금액", "金额"],
    'moon.amountAria': ["金額（NT$）", "Amount (NT$)", "金額（NT$）", "금액(NT$)", "金额（NT$）"],
    'moon.styleAria': ["燈籠樣式", "Lantern style", "灯籠の種類", "등불 종류", "灯笼样式"],
    'moon.styleAuto': ["依金額自動", "Auto by amount", "金額で自動", "금액에 따라 자동", "按金额自动"],
    'moon.styleManual': ["手動指定", "Picked manually", "手動で指定", "직접 지정", "手动指定"],
    'moon.style.paper': ["紙燈籠", "Paper lantern", "紙灯籠", "종이 등불", "纸灯笼"],
    'moon.style.red': ["紅燈籠", "Red lantern", "赤提灯", "붉은 등불", "红灯笼"],
    'moon.style.pomelo': ["柚子燈", "Pomelo lantern", "文旦灯籠", "포멜로 등불", "柚子灯"],
    'moon.style.palace': ["宮燈", "Palace lantern", "宮灯", "궁등", "宫灯"],
    'moon.style.rabbit': ["月兔燈", "Moon rabbit lantern", "月うさぎ灯籠", "달토끼 등불", "月兔灯"],
    'moon.style.fallback': ["燈籠", "lantern", "灯籠", "등불", "灯笼"],
    'moon.hang': ["掛上燈籠", "Hang lantern", "灯籠を掛ける", "등불 걸기", "挂上灯笼"],
    'moon.previewTitle': ["OBS 畫面預覽", "OBS preview", "OBS 画面プレビュー", "OBS 화면 미리보기", "OBS 画面预览"],
    'moon.previewAria': ["預覽內容", "Preview content", "プレビュー内容", "미리보기 내용", "预览内容"],
    'moon.previewLive': ["實際畫面", "Live view", "実際の画面", "실제 화면", "实际画面"],
    'moon.previewLanterns': ["五種燈籠示範", "All five lanterns", "5 種類の灯籠デモ", "등불 5종 데모", "五种灯笼示范"],
    'moon.previewCelebrate': ["滿月慶祝示範", "Full-moon celebration demo", "満月のお祝いデモ", "보름달 축하 데모", "满月庆祝示范"],
    'moon.previewCredits': ["謝幕名單", "Credits", "エンドロール", "엔딩 크레딧", "谢幕名单"],
    'moon.previewFrame': ["中秋活動預覽", "Mid-Autumn event preview", "中秋イベントのプレビュー", "추석 이벤트 미리보기", "中秋活动预览"],
    'moon.urlAll': ["月亮＋燈籠（整個畫面）", "Moon + lanterns (full screen)", "月＋灯籠（画面全体）", "달 + 등불(전체 화면)", "月亮＋灯笼（整个画面）"],
    'moon.urlMoon': ["只有月亮", "Moon only", "月のみ", "달만", "只有月亮"],
    'moon.urlLanterns': ["只有燈籠", "Lanterns only", "灯籠のみ", "등불만", "只有灯笼"],
    'moon.urlCredits': ["謝幕名單（平常透明，按下方「播放謝幕名單」才出現）", "Credits (transparent until you press “Play credits” below)", "エンドロール（普段は透明。下の「エンドロールを再生」を押すと表示）", "엔딩 크레딧(평소에는 투명, 아래 ‘엔딩 크레딧 재생’을 누르면 표시)", "谢幕名单（平常透明，按下方“播放谢幕名单”才出现）"],
    'moon.obsHint': ["在 OBS 新增「瀏覽器」來源貼上網址，寬高設 1920×1080。想把月亮和燈籠分開擺，就各建一個來源。", "In OBS, add a “Browser” source, paste the URL, and set it to 1920×1080. To place the moon and lanterns separately, create one source for each.", "OBS で「ブラウザ」ソースを追加して URL を貼り付け、サイズを 1920×1080 にします。月と灯籠を別々に配置したい場合は、それぞれソースを作成してください。", "OBS에서 ‘브라우저’ 소스를 추가해 URL을 붙여 넣고 크기를 1920×1080으로 설정하세요. 달과 등불을 따로 배치하려면 각각 소스를 만드세요.", "在 OBS 新增“浏览器”来源并粘贴网址，宽高设为 1920×1080。想把月亮和灯笼分开摆放，就各建一个来源。"],
    'moon.configTitle': ["活動設定", "Event settings", "イベント設定", "이벤트 설정", "活动设置"],
    'moon.titleLabel': ["標題", "Title", "タイトル", "제목", "标题"],
    'moon.doneTextLabel': ["達標後顯示的文字", "Text shown when the goal is reached", "目標達成時に表示する文字", "목표 달성 시 표시할 문구", "达标后显示的文字"],
    'moon.goalLabel': ["目標金額（NT$）", "Goal (NT$)", "目標金額（NT$）", "목표 금액(NT$)", "目标金额（NT$）"],
    'moon.baseLabel': ["起始金額（NT$）", "Starting amount (NT$)", "開始金額（NT$）", "시작 금액(NT$)", "起始金额（NT$）"],
    'moon.baseHint': ["起始金額＝活動開始前已經募到的錢（例如直播上顯示的募資進度），月亮會從這裡開始算。", "Starting amount = money already raised before the event (for example, the progress shown on stream). The moon starts counting from here.", "開始金額＝イベント開始前にすでに集まっている金額（配信で表示している募金の進捗など）。月はここから数え始めます。", "시작 금액 = 이벤트 시작 전에 이미 모인 금액(예: 방송에 표시 중인 모금 진행도)입니다. 달은 여기서부터 계산을 시작합니다.", "起始金额＝活动开始前已经筹到的钱（例如直播上显示的筹款进度），月亮会从这里开始算。"],
    'moon.tiersLabel': ["燈籠門檻（NT$ 起）", "Lantern thresholds (from NT$)", "灯籠のしきい値（NT$ 以上）", "등불 기준 금액(NT$ 이상)", "灯笼门槛（NT$ 起）"],
    'moon.tiersHint': ["「依金額自動」時，斗內金額達到哪個門檻就掛哪一種；改門檻會立刻套用到所有自動的燈籠。", "With “Auto by amount”, each donation gets the highest lantern whose threshold it reaches. Changing thresholds updates all auto lanterns immediately.", "「金額で自動」の場合、投げ銭の金額が到達したしきい値の灯籠が掛かります。しきい値を変えると、自動の灯籠すべてにすぐ反映されます。", "‘금액에 따라 자동’일 때는 후원 금액이 도달한 기준의 등불이 걸립니다. 기준을 바꾸면 자동 등불 모두에 즉시 적용됩니다.", "“按金额自动”时，打赏金额达到哪个门槛就挂哪一种；修改门槛会立刻套用到所有自动的灯笼。"],
    'moon.save': ["儲存設定", "Save settings", "設定を保存", "설정 저장", "保存设置"],
    'moon.listTitle': ["斗內紀錄", "Donations", "投げ銭の記録", "후원 기록", "打赏记录"],
    'moon.playCelebrate': ["播放滿月慶祝", "Play full-moon celebration", "満月のお祝いを再生", "보름달 축하 재생", "播放满月庆祝"],
    'moon.playCelebrateHint': ["達標時會自動播放一次；這裡可以手動再放", "Plays automatically once when the goal is reached; replay it here anytime", "目標達成時に自動で一度再生されます。ここから手動で再生できます", "목표 달성 시 한 번 자동 재생되며, 여기서 다시 재생할 수 있습니다", "达标时会自动播放一次；这里可以手动再放"],
    'moon.playCredits': ["播放謝幕名單", "Play credits", "エンドロールを再生", "엔딩 크레딧 재생", "播放谢幕名单"],
    'moon.clearAll': ["全部清除", "Clear all", "すべて消去", "모두 지우기", "全部清除"],
    'moon.empty': ["還沒有斗內紀錄", "No donations yet", "まだ投げ銭の記録はありません", "아직 후원 기록이 없습니다", "还没有打赏记录"],
    'moon.toast.invalid': ["請填寫名字與大於 0 的金額", "Enter a name and an amount above 0", "名前と 0 より大きい金額を入力してください", "이름과 0보다 큰 금액을 입력하세요", "请填写名字与大于 0 的金额"],
    'moon.toast.failed': ["操作失敗，請再試一次", "Something went wrong. Please try again.", "操作に失敗しました。もう一度お試しください。", "작업에 실패했습니다. 다시 시도해 주세요.", "操作失败，请再试一次"],
    'moon.toast.ended': ["中秋活動已結束", "The Mid-Autumn event has ended", "中秋イベントは終了しました", "추석 이벤트가 종료되었습니다", "中秋活动已结束"],
    'moon.toast.copied': ["已複製網址", "URL copied", "URL をコピーしました", "URL을 복사했습니다", "已复制网址"],
    'moon.toast.copyFailed': ["複製失敗，請手動選取網址", "Couldn’t copy. Please select the URL manually.", "コピーできませんでした。URL を手動で選択してください。", "복사하지 못했습니다. URL을 직접 선택하세요.", "复制失败，请手动选择网址"],
    'moon.toast.saved': ["活動設定已儲存", "Event settings saved", "イベント設定を保存しました", "이벤트 설정을 저장했습니다", "活动设置已保存"],
    'moon.toast.celebrate': ["滿月慶祝播放中", "Playing the full-moon celebration", "満月のお祝いを再生中", "보름달 축하 재생 중", "满月庆祝播放中"],
    'moon.toast.credits': ["謝幕名單開始播放", "Credits are rolling", "エンドロールを再生中", "엔딩 크레딧 재생 중", "谢幕名单开始播放"],
    'moon.confirmDelete.title': ["刪除這筆斗內？", "Delete this donation?", "この投げ銭を削除しますか？", "이 후원을 삭제할까요?", "删除这笔打赏？"],
    'moon.confirmDelete.impact': ["燈籠會從 OBS 畫面拿掉，月亮進度也會扣回去。", "The lantern is removed from the OBS overlay and the moon’s progress goes back down.", "灯籠が OBS 画面から外れ、月の進み具合も元に戻ります。", "등불이 OBS 화면에서 사라지고 달의 진행도도 다시 줄어듭니다.", "灯笼会从 OBS 画面移除，月亮进度也会扣回去。"],
    'moon.confirmClear.title': ["清除全部斗內紀錄？", "Clear all donations?", "投げ銭の記録をすべて消去しますか？", "후원 기록을 모두 지울까요?", "清除全部打赏记录？"],
    'moon.confirmClear.summary': ["OBS 上的燈籠會全部拿掉，月亮回到起始金額。", "All lanterns are removed from OBS and the moon returns to the starting amount.", "OBS の灯籠がすべて外れ、月は開始金額に戻ります。", "OBS의 등불이 모두 사라지고 달은 시작 금액으로 돌아갑니다.", "OBS 上的灯笼会全部移除，月亮回到起始金额。"],
    'moon.confirmClear.impact': ["這個動作無法復原；活動設定（標題、目標、起始金額）會保留。", "This can’t be undone. Event settings (title, goal, starting amount) are kept.", "この操作は元に戻せません。イベント設定（タイトル・目標・開始金額）は残ります。", "이 작업은 되돌릴 수 없습니다. 이벤트 설정(제목, 목표, 시작 금액)은 유지됩니다.", "这个操作无法恢复；活动设置（标题、目标、起始金额）会保留。"],
    'moon.confirmClear.phrase': ["清除", "Clear", "クリア", "지우기", "清除"],
    'moon.overlay.thanks': ["感謝 {name} 點亮{style}", "Thank you, {name}, for lighting a {style}!", "{name} さん、{style}をありがとう！", "{name} 님, {style} 고마워요!", "感谢 {name} 点亮{style}"],
    'moon.credits.heading': ["感 謝 名 單", "SPECIAL THANKS", "スペシャルサンクス", "고마운 분들", "感 谢 名 单"],
    'moon.credits.closing': ["謝謝每一份心意", "Thank you for every bit of support", "ひとつひとつの気持ちに感謝を", "보내주신 모든 마음에 감사드립니다", "谢谢每一份心意"],
    'moon.credits.summary': ["NT${total}・{count} 位", "NT${total} · {count} supporters", "NT${total}・{count} 名", "NT${total} · {count}명", "NT${total}・{count} 位"],
    ...
    'aiInstall.cancelInstall': ['取消安裝', 'Cancel install', 'インストールを中止', '설치 취소', '取消安装'],
    'aiInstall.cancelConfirm': ['確定取消？再按一次', 'Really cancel? Click again', '本当に中止しますか？もう一度押してください', '정말 취소할까요? 한 번 더 누르세요', '确定取消？再按一次'],
    'aiInstall.cancelling': ['正在取消…', 'Cancelling…', '中止しています…', '취소하는 중…', '正在取消…'],
    'aiInstall.cancellingStage': ['正在取消安裝，清除下載到一半的檔案…', 'Cancelling the install and removing partially downloaded files…', 'インストールを中止し、途中までダウンロードしたファイルを削除しています…', '설치를 취소하고 일부만 다운로드된 파일을 삭제하는 중…', '正在取消安装，清除下载到一半的文件…'],
    'aiInstall.cancelled': ['已取消安裝，下載到一半的檔案已清除。已經裝好的部分會保留，下次從缺的地方繼續。', 'Install cancelled and partial files removed. Parts that already finished are kept, so the next attempt continues from where it stopped.', 'インストールを中止し、途中のファイルを削除しました。完了済みの部分は残るので、次回は足りない部分から再開します。', '설치를 취소하고 일부만 받은 파일을 삭제했습니다. 이미 완료된 부분은 유지되어 다음에는 부족한 부분부터 이어서 진행합니다.', '已取消安装，下载到一半的文件已清除。已经装好的部分会保留，下次从缺的地方继续。'],
    ...
    'aiInstall.step.pipInstall': ['正在安裝 PyTorch 等套件（這一步沒有進度可顯示，通常要 3–15 分鐘）・已經過 {elapsed}', 'Installing PyTorch and other packages (no progress is reported for this step; it usually takes 3–15 min) · {elapsed} elapsed', 'PyTorch などのパッケージをインストール中（この段階は進捗を表示できません。通常 3〜15 分）・経過 {elapsed}', 'PyTorch 등 패키지 설치 중(이 단계는 진행률을 표시할 수 없으며 보통 3~15분 소요) · {elapsed} 경과', '正在安装 PyTorch 等套件（这一步没有进度可显示，通常要 3–15 分钟）・已经过 {elapsed}'],
    'aiInstall.step.pipInstallSlow': ['這一步比平常久（已經過 {elapsed}）。防毒軟體逐一掃描檔案時會特別慢，可以繼續等；想放棄的話按「取消安裝」，之後再重試。', 'This step is taking longer than usual ({elapsed} elapsed). Antivirus scanning each file can slow it down a lot — you can keep waiting, or press Cancel install and retry later.', 'この段階はいつもより時間がかかっています（経過 {elapsed}）。ウイルス対策ソフトがファイルを1つずつスキャンしていると特に遅くなります。このまま待つか、「インストールを中止」を押して後でやり直してください。', '이 단계가 평소보다 오래 걸리고 있습니다({elapsed} 경과). 백신 프로그램이 파일을 하나씩 검사하면 특히 느려집니다. 계속 기다리거나 설치 취소를 누르고 나중에 다시 시도하세요.', '这一步比平常久（已经过 {elapsed}）。杀毒软件逐一扫描文件时会特别慢，可以继续等；想放弃的话按“取消安装”，之后再重试。'],
    ...
    'aiJob.waitingGpu': ['等待段落分析釋放 GPU…', 'Waiting for section analysis to free the GPU…', 'セクション分析が GPU を解放するのを待っています…', '섹션 분석이 GPU를 해제하기를 기다리는 중…', '等待段落分析释放 GPU…'],
    ...
    'jizura.placementFull': ['置中', 'Center', '中央', '가운데', '居中'],
    'jizura.placementHint': ['預設偏右。裝飾效果一律鋪滿全畫面；歌詞會依每句版面的寬度盡量往側邊推，太寬的版面（例如跑馬燈）推不動就維持原位。', 'Right by default. Effects always cover the full screen; the lyrics shift toward the side as far as each layout allows, and very wide layouts (like marquees) stay where they are.', '既定は右寄せです。演出は常に全画面で、歌詞はレイアウトの幅に応じてできるだけ横へ寄せます。幅の広いレイアウト（マーキーなど）は動かせないのでそのままです。', '기본값은 오른쪽입니다. 효과는 항상 전체 화면이며, 가사는 레이아웃 폭이 허락하는 만큼 옆으로 옮깁니다. 아주 넓은 레이아웃(마키 등)은 그대로 둡니다.', '默认偏右。装饰效果一律铺满全画面；歌词会依每句版面的宽度尽量往侧边推，太宽的版面（例如跑马灯）推不动就维持原位。'],
    'jizura.motionLabel': ['動作節奏', 'Motion', '動きのリズム', '동작 리듬', '动作节奏'],
    'jizura.motionSmooth': ['流暢', 'Smooth', 'なめらか', '부드럽게', '流畅'],
    'jizura.motionKoma': ['作畫感', 'On twos', '作画感', '작화 느낌', '作画感'],
    'jizura.motionSmoothTitle': ['流暢：每一幀都更新動作', 'Smooth: motion updates every frame', 'なめらか：毎フレーム動きを更新します', '부드럽게: 매 프레임 동작을 갱신합니다', '流畅：每一帧都更新动作'],
    'jizura.motionKomaTitle': ['作畫感：每秒 12 張的頓挫（日系文字 PV 常用的「一拍二」）', 'On twos: 12 drawings per second, the stepped feel common in Japanese lyric videos', '作画感：毎秒12枚のコマ打ち（文字PVでよく使われる2コマ打ち）', '작화 느낌: 초당 12장의 끊어지는 움직임(일본 가사 PV에서 흔한 2코마)', '作画感：每秒 12 张的顿挫（日系文字 PV 常用的「一拍二」）'],
    'jizura.motionHint': ['作畫感是刻意的一格一格動（每秒 12 張），不是掉幀；想要滑順就選流暢。', 'On twos steps deliberately (12 drawings per second) — it is not dropped frames. Pick Smooth for fluid motion.', '作画感はわざとコマ送りにしています（毎秒12枚）。コマ落ちではありません。なめらかにしたい場合は「なめらか」を選んでください。', '작화 느낌은 일부러 끊어 움직이는 것(초당 12장)이며 프레임 드롭이 아닙니다. 부드럽게 하려면 "부드럽게"를 선택하세요.', '作画感是刻意的一格一格动（每秒 12 张），不是掉帧；想要顺滑就选流畅。'],
    ...
    'publicRequest.customSlugLabel': ['自訂網址（選填）', 'Custom link (optional)', 'カスタムリンク（任意）', '맞춤 링크(선택 사항)', '自定义链接（选填）'],
    'publicRequest.customSlugPlaceholder': ['例如 my-channel-name', 'e.g. my-channel-name', '例：my-channel-name', '예: my-channel-name', '例如 my-channel-name'],
    'publicRequest.customSlugHint': [
      '只能是英文字母、數字、- 或 _，長度 8～64 個字元；套用後舊連結立即失效，且無法保證這個字串沒被別人搶先用掉。',
      'Letters, digits, - or _ only, 8–64 characters. Applying it invalidates the old link immediately, and there is no guarantee someone else hasn’t already claimed this string.',
      '英数字・-・_ のみ、8～64文字。適用すると旧リンクは即座に無効になり、この文字列が既に他の人に使われていないことは保証されません。',
      '영문자, 숫자, -, _ 만 사용, 길이 8~64자. 적용하면 기존 링크가 즉시 무효화되며, 이 문자열을 다른 사람이 먼저 사용하지 않았다는 보장은 없습니다.',
      '只能是英文字母、数字、- 或 _，长度 8～64 个字符；套用后旧链接立即失效，且无法保证这个字符串没被别人抢先用掉。',
    ],
    'publicRequest.applySlug': ['套用', 'Apply', '適用', '적용', '套用'],
    'publicRequest.slugApplied': ['已套用自訂網址', 'Custom link applied', 'カスタムリンクを適用しました', '맞춤 링크를 적용했습니다', '已套用自定义链接'],
    'publicRequest.slugApplyFailed': ['套用自訂網址失敗', 'Could not apply the custom link', 'カスタムリンクを適用できませんでした', '맞춤 링크를 적용하지 못했습니다', '套用自定义链接失败'],
    'publicRequest.slugRemaining': ['還可以自訂 {count} 次', '{count} custom link change(s) left', 'カスタムリンクはあと{count}回変更できます', '맞춤 링크 변경 가능 횟수: {count}회', '还可以自定义 {count} 次'],
    'publicRequest.slugExhausted': ['自訂網址機會已用完，無法再變更', 'No custom link changes left', 'カスタムリンクの変更回数を使い切りました', '맞춤 링크 변경 횟수를 모두 사용했습니다', '自定义链接机会已用完，无法再变更'],
