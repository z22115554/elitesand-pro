...
(function (root, factory) {
  ...
  const ROWS = {
    ...
    'appUpdate.available': ['發現新版本 {version}。按「重新啟動並更新」後，程式才會處理更新。', 'Version {version} is available. The update runs only after you choose “Restart and update.”', '新しいバージョン {version} があります。「再起動して更新」を選んだ後に処理します。', '새 버전 {version}을 사용할 수 있습니다. “다시 시작하고 업데이트”를 선택한 후 처리합니다.', '发现新版本 {version}。选择“重新启动并更新”后才会处理。'],
    ...
    'aiJob.cpuFallback': ['WebGPU 不可用，正在改用 CPU', 'WebGPU unavailable; switching to CPU', 'WebGPU を使用できないため CPU へ切り替え中', 'WebGPU를 사용할 수 없어 CPU로 전환 중', 'WebGPU 不可用，正在改用 CPU'],
    ...
    'tour.advanced.step.obsDualAudio.hint': ['只播伴奏、看首頁逐字歌詞跟拍是最不容易出錯的做法；需要原唱參考時再照教學設定監聽。', 'Playing just the instrumental and following the word-by-word lyrics on the home page is the least error-prone setup. Set up monitoring per the guide only when you need the reference vocals.', '伴奏だけを再生し、ホームの逐語歌詞で合わせるのが最も間違いにくい方法です。原曲ガイドが必要なときだけガイドに従ってモニターを設定します。', '반주만 재생하고 홈의 한 글자씩 가사를 따라가는 것이 가장 실수 적은 방법입니다. 원곡 가이드가 필요할 때만 가이드대로 모니터링을 설정하세요.', '只播伴奏、看首页逐字歌词跟拍是最不容易出错的做法；需要原唱参考时再照教程设置监听。'],
