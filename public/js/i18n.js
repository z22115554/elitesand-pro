...
(function (root, factory) {
  ...
  const ROWS = {
    ...
    'nav.sections': ['段落設計', 'Section design', 'セクション設計', '섹션 디자인', '段落设计'],
    'sections.experimentalBadge': ['實驗性', 'Experimental', '実験的', '실험적', '实验性'],
    'sections.analysisTitle': ['段落分析', 'Section analysis', 'セクション分析', '섹션 분석', '段落分析'],
    'sections.analysisDesc': [
      '自動判斷主歌／副歌／間奏等段落的功能，目前還在獨立研究階段，尚未串接進正式資料流。',
      'Automatically detecting verse/chorus/bridge sections is still an independent research effort and is not yet wired into the production data flow.',
      '主歌・サビ・間奏などのセクションを自動判定する機能は、まだ独立した研究段階にあり、正式なデータフローには接続されていません。',
      '주고/후렴/간주 등 섹션을 자동으로 판단하는 기능은 아직 별도 연구 단계이며, 정식 데이터 흐름에는 연결되어 있지 않습니다.',
      '自动判断主歌／副歌／间奏等段落的功能，目前还在独立研究阶段，尚未接入正式数据流。',
    ],
    'sections.analysisPlaceholder': ['尚未串接，開發中。', 'Not wired up yet — in development.', '未接続、開発中です。', '아직 연결되지 않았습니다. 개발 중입니다.', '尚未接入，开发中。'],
    'sections.pairingTitle': ['段落搭配', 'Section pairing', 'セクション割り当て', '섹션 매칭', '段落搭配'],
    'sections.pairingDesc': [
      '依段落切換不同的歌詞模板，例如副歌換一個比較有存在感的版面、主歌維持安靜。段落資料目前是手動測試用，還沒接上正式的段落分析結果。',
      'Switch to a different lyric template per section — e.g. a bolder layout for the chorus while the verse stays quiet. Section data here is manual test data, not yet the real analysis result.',
      'セクションごとに異なる歌詞テンプレートに切り替えます（例：サビは存在感のあるレイアウトに、主歌は静かなままに）。ここのセクションデータは手動のテスト用で、正式な分析結果ではありません。',
      '섹션별로 다른 가사 템플릿으로 전환합니다（예: 후렴은 존재감 있는 레이아웃으로, 벌스는 조용하게 유지）. 여기의 섹션 데이터는 수동 테스트용이며 정식 분석 결과가 아닙니다.',
      '依段落切换不同的歌词模板，例如副歌换一个比较有存在感的版面、主歌维持安静。段落数据目前是手动测试用，还没接上正式的段落分析结果。',
    ],
    'sections.enableToggle': ['啟用段落搭配（測試中）', 'Enable section pairing (testing)', 'セクション割り当てを有効化（テスト中）', '섹션 매칭 활성화（테스트 중）', '启用段落搭配（测试中）'],
    'sections.testTrackNote': [
      '目前只掛了一首測試曲的段落資料（手動輸入，非正式分析結果），播放其他歌曲時這個功能不會有作用。',
      'Only one test track currently has section data (manually entered, not a real analysis result); this has no effect on any other song.',
      '現在はテスト曲 1 曲分のセクションデータのみ（手入力、正式な分析結果ではありません）。他の曲を再生してもこの機能は働きません。',
      '현재는 테스트 곡 한 곡의 섹션 데이터만 있습니다（수동 입력, 정식 분석 결과 아님）. 다른 곡을 재생할 때는 이 기능이 작동하지 않습니다.',
      '目前只挂了一首测试曲的段落数据（手动输入，非正式分析结果），播放其他歌曲时这个功能不会有作用。',
    ],
    'sections.viewHint': [
      '這裡不做即時預覽，效果請直接開 /display 分頁或 OBS 來源觀察。',
      'There is no live preview here — open the /display tab or your OBS source to see the effect.',
      'ここではリアルタイムプレビューは行いません。効果は /display タブか OBS ソースで直接ご確認ください。',
      '여기서는 실시간 미리보기를 제공하지 않습니다. 효과는 /display 탭이나 OBS 소스에서 직접 확인하세요.',
      '这里不做即时预览，效果请直接打开 /display 分页或 OBS 来源观察。',
    ],
    'sections.followCurrent': ['跟隨目前模板', 'Follow current template', '現在のテンプレートに従う', '현재 템플릿 따르기', '跟随目前模板'],
    'sections.label.intro': ['前奏', 'Intro', 'イントロ', '인트로', '前奏'],
    'sections.label.verse': ['主歌', 'Verse', 'Aメロ', '벌스', '主歌'],
    'sections.label.pre_chorus': ['副歌前（蓄力）', 'Pre-chorus', 'Bメロ（サビ前）', '프리코러스', '副歌前（蓄力）'],
    'sections.label.chorus': ['副歌', 'Chorus', 'サビ', '후렴', '副歌'],
    'sections.label.bridge': ['Bridge（過渡）', 'Bridge', 'ブリッジ', '브릿지', 'Bridge（过渡）'],
    'sections.label.post_chorus': ['副歌後', 'Post-chorus', 'サビ後', '후렴 이후', '副歌后'],
    'sections.label.outro': ['尾奏', 'Outro', 'アウトロ', '아웃트로', '尾奏'],
    ...
    'guide.obsTaskBefore': ['把', 'Drag the', '「', '"', '把'],
    'guide.obsTaskAfter': ['方塊拖進 OBS（或貼網址）', 'tile into OBS (or paste the URL)', '」タイルを OBS にドラッグ（または URL を貼り付け）', '" 타일을 OBS로 드래그（또는 URL 붙여넣기）', '方块拖进 OBS（或粘贴网址）'],
    ...
    'library.playlists.ariaLabel': ['收藏歌單', 'Collections', 'コレクション', '컬렉션', '收藏歌单'],
    'library.playlists.all': ['全部', 'All', 'すべて', '전체', '全部'],
    'library.playlists.new': ['＋ 新收藏歌單', '+ New collection', '＋ 新規コレクション', '+ 새 컬렉션', '＋ 新收藏歌单'],
    'library.playlists.namePlaceholder': ['收藏歌單名稱', 'Collection name', 'コレクション名', '컬렉션 이름', '收藏歌单名称'],
    'library.playlists.createEmpty': ['建立', 'Create', '作成', '만들기', '建立'],
    'library.playlists.createFromQueue': ['用目前播放清單建立', 'Create from current playlist', '現在のプレイリストから作成', '현재 재생목록으로 만들기', '用当前播放列表建立'],
    'library.playlists.loadAll': ['全部載入播放清單', 'Load all into playlist', 'すべてプレイリストへ', '전체 재생목록에 추가', '全部载入播放列表'],
    'library.playlists.rename': ['重新命名', 'Rename', '名前を変更', '이름 변경', '重新命名'],
    'library.playlists.delete': ['刪除收藏歌單', 'Delete collection', 'コレクションを削除', '컬렉션 삭제', '删除收藏歌单'],
    'library.playlists.summary': ['{count} 首', '{count} songs', '{count} 曲', '{count}곡', '{count} 首'],
    'library.playlists.summaryMissing': ['{count} 首（{missing} 首已不在媒體庫）', '{count} songs ({missing} no longer in library)', '{count} 曲（{missing} 曲はライブラリにありません）', '{count}곡（{missing}곡은 라이브러리에 없음）', '{count} 首（{missing} 首已不在媒体库）'],
    'library.playlists.addTo': ['加入收藏歌單', 'Add to collection', 'コレクションに追加', '컬렉션에 추가', '加入收藏歌单'],
    'library.playlists.removeFrom': ['移出收藏歌單', 'Remove from collection', 'コレクションから外す', '컬렉션에서 제거', '移出收藏歌单'],
    'library.playlists.noneYet': ['還沒有收藏歌單，先按「＋ 新收藏歌單」。', 'No collections yet — tap "+ New collection" first.', 'コレクションはまだありません。「＋ 新規コレクション」から作成してください。', '아직 컬렉션이 없습니다. 먼저 "+ 새 컬렉션"을 누르세요.', '还没有收藏歌单，先按「＋ 新收藏歌单」。'],
    'library.playlists.empty': ['這個收藏歌單還沒有歌。切回「全部」，在任一首歌按「＋」加入。', 'This collection is empty. Switch to "All" and press "+" on any song.', 'このコレクションは空です。「すべて」に戻り、曲の「＋」を押してください。', '이 컬렉션은 비어 있습니다. "전체"로 돌아가 곡의 "+"를 누르세요.', '这个收藏歌单还没有歌。切回「全部」，在任一首歌按「＋」加入。'],
    'library.playlists.created': ['已建立收藏歌單「{name}」', 'Created collection "{name}"', 'コレクション「{name}」を作成しました', '컬렉션 "{name}"을(를) 만들었습니다', '已建立收藏歌单「{name}」'],
    'library.playlists.createdFromQueue': ['已建立收藏歌單「{name}」（{count} 首）', 'Created collection "{name}" ({count} songs)', 'コレクション「{name}」を作成しました（{count} 曲）', '컬렉션 "{name}"을(를) 만들었습니다（{count}곡）', '已建立收藏歌单「{name}」（{count} 首）'],
    'library.playlists.queueEmpty': ['目前播放清單是空的', 'The current playlist is empty', '現在のプレイリストは空です', '현재 재생목록이 비어 있습니다', '当前播放列表是空的'],
    'library.playlists.deleteTitle': ['刪除收藏歌單「{name}」？', 'Delete collection "{name}"?', 'コレクション「{name}」を削除しますか？', '컬렉션 "{name}"을(를) 삭제할까요?', '删除收藏歌单「{name}」？'],
    'library.playlists.deleteSummary': ['只會刪除這個收藏歌單，不會刪除任何歌曲或媒體庫紀錄。', 'Only the collection is deleted; songs and library records are kept.', 'コレクションのみ削除され、曲やライブラリの記録は残ります。', '컬렉션만 삭제되며 곡과 라이브러리 기록은 유지됩니다.', '只会删除这个收藏歌单，不会删除任何歌曲或媒体库记录。'],
    'library.playlists.deleteImpact': ['收藏歌單內的順序與組合無法復原。', 'The order and grouping in this collection cannot be restored.', 'コレクション内の順序と組み合わせは元に戻せません。', '컬렉션의 순서와 구성은 복구할 수 없습니다.', '收藏歌单内的顺序与组合无法复原。'],
    'library.playlists.loadTitle': ['載入「{name}」到播放清單？', 'Load "{name}" into the playlist?', '「{name}」をプレイリストに読み込みますか？', '"{name}"을(를) 재생목록에 불러올까요?', '载入「{name}」到播放列表？'],
    'library.playlists.loadSummary': ['會依收藏歌單順序把 {count} 首歌加到播放清單末端。', '{count} songs will be appended to the playlist in collection order.', 'コレクションの順に {count} 曲をプレイリストの末尾に追加します。', '컬렉션 순서대로 {count}곡을 재생목록 끝에 추가합니다.', '会依收藏歌单顺序把 {count} 首歌加到播放列表末端。'],
    'library.playlists.loadImpact': ['音檔已不在的歌會重新下載；已在播放清單的 {dup} 首會再加一次。', 'Songs without a local audio file will be re-downloaded; {dup} already in the playlist will be added again.', '音声ファイルがない曲は再ダウンロードされます。プレイリストにある {dup} 曲は再度追加されます。', '오디오 파일이 없는 곡은 다시 내려받습니다. 이미 재생목록에 있는 {dup}곡은 다시 추가됩니다.', '音频已不在的歌会重新下载；已在播放列表的 {dup} 首会再加一次。'],
    'library.playlists.dragToReorder': ['拖曳調整順序', 'Drag to reorder', 'ドラッグで並べ替え', '드래그하여 순서 변경', '拖曳调整顺序'],
    'library.playlists.loadConfirm': ['載入', 'Load', '読み込む', '불러오기', '载入'],
    'library.playlists.loadQueued': ['已排入 {count} 首，逐首加入中…', 'Queued {count} songs, adding one by one…', '{count} 曲をキューに入れました。順に追加中…', '{count}곡을 대기열에 넣었습니다. 순서대로 추가 중…', '已排入 {count} 首，逐首加入中…'],
    'library.playlists.serverNoAck': ['伺服器沒有確認', 'The server did not confirm the change', 'サーバーが変更を確認しませんでした', '서버가 변경을 확인하지 않았습니다', '服务器没有确认'],
    'library.playlists.error.name_required': ['請輸入收藏歌單名稱', 'Enter a collection name', 'コレクション名を入力してください', '컬렉션 이름을 입력하세요', '请输入收藏歌单名称'],
    'library.playlists.error.too_many_playlists': ['收藏歌單數量已達上限', 'Collection limit reached', 'コレクション数が上限に達しました', '컬렉션 수가 한도에 도달했습니다', '收藏歌单数量已达上限'],
    'library.playlists.error.playlist_full': ['這個收藏歌單已滿', 'This collection is full', 'このコレクションは満杯です', '이 컬렉션이 가득 찼습니다', '这个收藏歌单已满'],
    'library.playlists.error.not_found': ['找不到這個收藏歌單', 'Collection not found', 'コレクションが見つかりません', '컬렉션을 찾을 수 없습니다', '找不到这个收藏歌单'],
    ...
    // 跟 i18n-auto.js 裡同一句 zh-TW 來源字串共用同一組翻譯，避免重複鍵翻譯不一致。
    'home.instrumentalVolume': ['伴奏音量', 'Instrumental volume', '伴奏の音量', '반주 음량', '伴奏音量'],
    'home.vocalsVolume': ['人聲音量', 'Vocals volume', 'ボーカルの音量', '보컬 음량', '人声音量'],
    'home.unifiedVolume': ['統一音量（響度標準化）', 'Unified volume (loudness normalization)', '統一音量（ラウドネス正規化）', '통합 볼륨(음량 정규화)', '统一音量（响度标准化）'],
    'offset.resetToZero': ['歸零', 'Reset to zero', 'ゼロに戻す', '0으로 초기화', '归零'],
    'home.unifiedVolumeHint': ['依每首歌的實測響度校正播放增益；關閉時保留原始音量', 'Correct playback gain according to the measured loudness of each song; retain the original volume when turned off', '各曲の測定されたラウドネスに応じて再生ゲインを修正します。オフにしても元の音量を維持する', '각 노래의 측정된 음량에 따라 재생 게인을 수정합니다. 꺼도 원래 볼륨 유지', '依每首歌的实测响度校正播放增益；关闭时保留原始音量'],
    ...
    'home.ai.intro': ['用 AI 把歌曲拆成人聲和伴奏。已分離歌曲播放時會自動使用伴奏＋人聲雙軌；新使用者的人聲預設 20%，可在播放區隨時調高或調低。', 'Use AI to split songs into vocal and instrumental tracks. Separated songs automatically use both stems during playback; vocals start at 20% for new users and can be raised or lowered in the playback controls.', 'AIで曲をボーカルと伴奏に分離します。分離済みの曲は再生時に自動で2トラックを使用し、新規ユーザーのボーカル音量は20%が初期値です。再生コントロールでいつでも上げ下げできます。', 'AI로 곡을 보컬과 반주 트랙으로 나눕니다. 분리된 곡은 재생할 때 자동으로 두 트랙을 사용하며, 새 사용자의 보컬 음량은 기본 20%입니다. 재생 컨트롤에서 언제든 올리거나 내릴 수 있습니다.', '用 AI 把歌曲拆成人声和伴奏。已分离歌曲播放时会自动使用伴奏＋人声双轨；新用户的人声默认 20%，可在播放区随时调高或调低。'],
    ...
    'help.ai.afterReady': ['不用另外開模式。播到「已分離」歌曲時會自動使用伴奏＋人聲雙軌；新使用者的人聲預設 20%，正在播放區會出現人聲音量，可自行調高、調低或拉到 0。沒分離的歌照原樣播放。', 'There is no mode to turn on. A separated song automatically uses the instrumental + vocal stems. For new users, vocals start at 20%; the Now Playing area shows a vocal-volume slider you can raise, lower, or set to 0. Unseparated songs play normally.', '別のモードをオンにする必要はありません。「分離済み」の曲は自動で伴奏＋ボーカルの2トラックを使います。新規ユーザーのボーカル音量は20%が初期値で、「再生中」エリアのボーカル音量から上げ下げしたり0にしたりできます。未分離の曲は通常どおり再生します。', '별도의 모드를 켤 필요가 없습니다. ‘분리됨’ 곡은 자동으로 반주＋보컬 두 트랙을 사용합니다. 새 사용자의 보컬 음량은 기본 20%이며 ‘재생 중’ 영역의 보컬 음량에서 올리거나 내리거나 0으로 만들 수 있습니다. 분리되지 않은 곡은 평소대로 재생됩니다.', '不用另外开启模式。播放到“已分离”歌曲时会自动使用伴奏＋人声双轨；新用户的人声默认 20%，正在播放区会出现人声音量，可自行调高、调低或拉到 0。没分离的歌照常播放。'],
    ...
    'help.dualAudio.appPart': ['已分離歌曲會自動以伴奏＋人聲雙軌播放；新使用者的人聲預設 20%，可在首頁「正在播放」區調整。要讓觀眾只聽伴奏、自己保留導唱，請使用「連線與系統」裡的「雙路音訊路由」。', 'Separated songs automatically play as instrumental + vocal stems. Vocals start at 20% for new users and can be adjusted in Now Playing. To let the audience hear only the instrumental while you keep the guide vocal, use Dual audio routing under Connection & System.', '分離済みの曲は自動で伴奏＋ボーカルの2トラック再生になります。新規ユーザーのボーカル音量は20%が初期値で、ホームの「再生中」で調整できます。視聴者には伴奏だけ、自分にはガイドボーカルを残すには、「接続とシステム」の「デュアル音声ルーティング」を使います。', '분리된 곡은 자동으로 반주＋보컬 두 트랙으로 재생됩니다. 새 사용자의 보컬 음량은 기본 20%이며 홈의 ‘재생 중’에서 조절할 수 있습니다. 시청자에게는 반주만, 본인에게는 가이드 보컬을 남기려면 ‘연결 및 시스템’의 ‘이중 오디오 라우팅’을 사용하세요.', '已分离歌曲会自动以伴奏＋人声双轨播放；新用户的人声默认 20%，可在首页“正在播放”区调整。要让观众只听伴奏、自己保留导唱，请使用“连接与系统”里的“双路音频路由”。'],
    'help.dualAudio.referenceRouteLabel': ['雙路音訊怎麼接', 'How to connect dual audio', 'デュアル音声の接続方法', '이중 오디오 연결 방법', '双路音频怎么接'],
    'help.dualAudio.referenceRoute': ['在「雙路音訊路由」卡先選「觀眾／OBS 裝置」和「主播耳機裝置」，再開啟路由。一般歌曲兩路都會播放；已分離歌曲的伴奏送兩路，人聲只留在主播耳機。', 'In the Dual audio routing card, choose the Audience / OBS device and the Streamer headphone device, then enable routing. Normal songs play on both paths; for separated songs, the instrumental goes to both paths while vocals stay in the streamer headphones.', '「デュアル音声ルーティング」カードで「視聴者／OBSデバイス」と「配信者用ヘッドホンデバイス」を選び、ルーティングを有効にします。通常の曲は両方へ出力され、分離済みの曲は伴奏が両方へ、ボーカルは配信者のヘッドホンだけへ送られます。', '‘이중 오디오 라우팅’ 카드에서 ‘시청자/OBS 장치’와 ‘스트리머 헤드폰 장치’를 선택한 뒤 라우팅을 켜세요. 일반 곡은 두 경로 모두로 재생되고, 분리된 곡은 반주가 두 경로로 가며 보컬은 스트리머 헤드폰에만 남습니다.', '在“双路音频路由”卡先选“观众／OBS 设备”和“主播耳机设备”，再开启路由。一般歌曲两路都会播放；已分离歌曲的伴奏送两路，人声只留在主播耳机。'],
    'help.dualAudio.obsWayLabel': ['OBS 要收哪一路', 'Which path OBS should capture', 'OBSで取り込む経路', 'OBS가 캡처할 경로', 'OBS 要收哪一路'],
    'help.dualAudio.obsWay': ['OBS 只擷取「觀眾／OBS 裝置」，不要再把「主播耳機裝置」同時收進直播。OBS 與 Windows 兩邊的取樣率都設成 44100Hz，可降低長時間播放漂移。', 'Have OBS capture only the Audience / OBS device; do not also capture the Streamer headphone device into the stream. Set both OBS and Windows audio to 44100Hz to reduce long-session drift.', 'OBSでは「視聴者／OBSデバイス」だけを取り込み、「配信者用ヘッドホンデバイス」は配信へ重ねて取り込まないでください。OBSとWindowsの両方を44100Hzに設定すると、長時間再生時のズレを抑えられます。', 'OBS는 ‘시청자/OBS 장치’만 캡처하고 ‘스트리머 헤드폰 장치’까지 방송에 함께 캡처하지 마세요. OBS와 Windows 오디오를 모두 44100Hz로 설정하면 장시간 재생 시 어긋남을 줄일 수 있습니다.', 'OBS 只采集“观众／OBS 设备”，不要再把“主播耳机设备”同时收进直播。OBS 与 Windows 两边的采样率都设成 44100Hz，可降低长时间播放漂移。'],
    'help.dualAudio.simplestAlternativeLabel': ['不需要分流時', 'When you do not need separate routing', '分流が不要な場合', '분리 라우팅이 필요 없을 때', '不需要分流时'],
    'help.dualAudio.simplestAlternative': ['可以不開雙路音訊路由；已分離歌曲仍會自動雙軌播放。若不想聽導唱，把首頁的人聲音量拉到 0% 即可。', 'You can leave Dual audio routing off; separated songs still use both stems automatically. If you do not want the guide vocal, set the vocal volume on the home screen to 0%.', 'デュアル音声ルーティングはオフのままでも構いません。分離済みの曲は引き続き自動で2トラック再生されます。ガイドボーカルが不要なら、ホームのボーカル音量を0%にしてください。', '이중 오디오 라우팅은 켜지 않아도 됩니다. 분리된 곡은 계속 자동으로 두 트랙을 사용합니다. 가이드 보컬이 필요 없다면 홈 화면의 보컬 음량을 0%로 설정하세요.', '可以不开双路音频路由；已分离歌曲仍会自动双轨播放。若不想听导唱，把首页的人声音量拉到 0% 即可。'],
    ...
    'home.lyricNow.noTimeline': ['這首歌詞沒有時間軸', 'This lyric has no timeline', 'この歌詞にはタイムラインがありません', '이 가사에는 타임라인이 없습니다', '这首歌词没有时间轴'],
    ...
    'playlist.separatedAutoTitle': ['已分離人聲，播放時會自動使用伴奏＋人聲雙軌', 'Vocals separated; playback automatically uses instrumental + vocal stems', 'ボーカル分離済み。再生時は自動で伴奏＋ボーカルの2トラックを使用します', '보컬 분리됨. 재생 시 자동으로 반주＋보컬 두 트랙을 사용합니다', '已分离人声，播放时会自动使用伴奏＋人声双轨'],
    ...
    'settings.lyrics.displayModeTitle': ['歌詞顯示模式', 'Lyric display mode', '歌詞表示モード', '가사 표시 모드', '歌词显示模式'],
    'settings.lyrics.languageDisplay': ['語言顯示', 'Lyrics language display', '歌詞の言語表示', '가사 언어 표시', '语言显示'],
    // 跟 i18n-auto.js 裡同一句 zh-TW 來源字串共用同一組翻譯，避免重複鍵翻譯不一致。
    'settings.lyrics.allModes': ['全部（原文＋拼音＋諧音）', 'All (original + romanization + phonetic guide)', 'すべて（原文＋ローマ字＋発音ガイド）', '전체(원문＋로마자＋발음 가이드)', '全部（原文＋拼音＋谐音）'],
    'settings.lyrics.romanizationHint': ['諧音（中文近似音）只對日文／韓文有效；中文歌一律只顯示拼音（羅馬字）。', 'The phonetic guide (a Chinese sound-alike approximation) only applies to Japanese/Korean; Chinese songs always show romanization (pinyin) only.', '発音ガイド（中国語の近似発音）は日本語／韓国語にのみ対応しています。中国語の曲は常にローマ字（拼音）のみ表示されます。', '발음 가이드(중국어 근사 발음)는 일본어/한국어에만 적용됩니다. 중국어 곡은 항상 로마자만 표시됩니다.', '谐音（中文近似音）只对日文/韩文有效；中文歌一律只显示拼音（罗马字）。'],
    'settings.lyrics.introCountdownHint': ['第一句歌詞前顯示脈動', 'Show a pulse before the first line', '最初の歌詞の前に脈動を表示', '첫 번째 가사 앞에 맥동 표시', '第一句歌词前显示脉动'],
    // 跟 i18n-auto.js 裡同一句 zh-TW 來源字串共用同一組翻譯（那份原本日文譯錯——
    // 「簡略化から従来型へ」意思是「從簡化到傳統／舊型」，完全沒表達「簡體字轉繁體字」，
    // 已在 i18n-auto.js 一併修正）。
    'settings.lyrics.convertTraditional': ['簡轉繁', 'Simplified to Traditional', '簡体字→繁体字変換', '간체→번체 변환', '简转繁'],
    'settings.lyrics.convertTraditionalHint': ['把簡體歌詞顯示成繁體（只轉原文，拼音/諧音不變）。網易雲/QQ 多為簡體時好用', 'Displays Simplified Chinese lyrics as Traditional (only the original text is converted; romanization/phonetic guide stay unchanged). Useful since NetEase Cloud Music/QQ Music lyrics are often Simplified.', '簡体字の歌詞を繁体字で表示します（原文のみ変換され、ローマ字／発音ガイドは変わりません）。網易雲や QQ音楽の歌詞は簡体字が多いため便利です。', '간체자 가사를 번체자로 표시합니다(원문만 변환되며 로마자/발음 가이드는 그대로입니다). 왕이윈뮤직/QQ뮤직 가사가 간체자인 경우가 많아 유용합니다.', '把简体歌词显示成繁体（只转原文，拼音/谐音不变）。网易云/QQ 多为简体时好用'],
    ...
    'preview.obsHint': ['要放進 OBS：去「連線與系統」把「歌詞畫面」方塊拖進 OBS，或複製此網址貼進瀏覽器來源。歌詞設定會自動同步，不需要更新來源。', 'To show this in OBS: drag the "Lyrics display" tile from Connection & system into OBS, or paste this URL into a Browser Source. Lyric settings sync automatically.', 'OBS に表示するには：「接続とシステム」の「歌詞画面」タイルを OBS にドラッグするか、この URL をブラウザソースに貼り付けます。歌詞設定は自動で同期します。', 'OBS에 표시하려면: "연결 및 시스템"의 "가사 화면" 타일을 OBS로 드래그하거나 이 URL을 브라우저 소스에 붙여넣으세요. 가사 설정은 자동으로 동기화됩니다.', '要放进 OBS：去“连接与系统”把“歌词画面”方块拖进 OBS，或复制此网址粘贴进浏览器来源。歌词设定会自动同步，不需要更新来源。'],
    ...
    'system.obsLauncherTitle': ['拖到 OBS 直接建立來源（實驗性）', 'Drag into OBS to create the source (experimental)', 'OBS にドラッグしてソースを作成（実験的）', 'OBS로 드래그하여 소스 만들기（실험적）', '拖到 OBS 直接创建来源（实验性）'],
    'system.obsLauncherDesc': ['把下面的方塊拖進 OBS 的預覽畫面，會自動新增一個對應的瀏覽器來源、尺寸等於畫布。這種來源不綁 port：之後 Elitesand Pro 換了連接埠，OBS 那邊也不用改。', 'Drag a tile below onto the OBS preview to add a matching Browser Source sized to the canvas. These sources are port-independent: if Elitesand Pro later runs on another port, OBS needs no change.', '下のタイルを OBS のプレビューにドラッグすると、キャンバスサイズのブラウザソースが自動で追加されます。このソースはポートに依存しません。後で Elitesand Pro のポートが変わっても OBS 側の変更は不要です。', '아래 타일을 OBS 미리보기로 드래그하면 캔버스 크기의 브라우저 소스가 자동으로 추가됩니다. 이 소스는 포트에 묶이지 않아, 나중에 Elitesand Pro 포트가 바뀌어도 OBS를 고칠 필요가 없습니다.', '把下面的方块拖进 OBS 的预览画面，会自动新增一个对应的浏览器来源、尺寸等于画布。这种来源不绑 port：之后 Elitesand Pro 换了端口，OBS 那边也不用改。'],
    'system.obsLauncherLyrics': ['歌詞畫面', 'Lyrics display', '歌詞画面', '가사 화면', '歌词画面'],
    'system.obsLauncherSetlist': ['歌單畫面', 'Setlist display', 'セットリスト画面', '세트리스트 화면', '歌单画面'],
    'system.obsLauncherCopyPath': ['複製檔案路徑', 'Copy file path', 'ファイルパスをコピー', '파일 경로 복사', '复制文件路径'],
    'system.obsLauncherHintFile': ['拖不進去的話：在 OBS 新增「瀏覽器」來源 → 勾「本機檔案」→ 選上面這個檔案。', 'If dragging does not work: add a Browser Source in OBS → check "Local file" → pick the file above.', 'ドラッグできない場合：OBS でブラウザソースを追加 → 「ローカルファイル」にチェック → 上のファイルを選択。', '드래그가 안 되면: OBS에서 브라우저 소스 추가 → "로컬 파일" 체크 → 위 파일 선택.', '拖不进去的话：在 OBS 新增“浏览器”来源 → 勾“本地文件” → 选上面这个文件。'],
    'system.obsLauncherHintUrl': ['目前是用瀏覽器開面板，拖出去的是網址（會綁目前的 port）；要不綁 port 的版本請用桌面版拖，或複製上面的檔案路徑到 OBS「本機檔案」。', 'You are using the panel in a browser, so the drag carries a URL (bound to the current port). For the port-independent version, drag from the desktop app or copy the file path above into an OBS "Local file" Browser Source.', 'ブラウザで開いているため、ドラッグされるのは URL（現在のポートに固定）です。ポートに依存しない版はデスクトップ版からドラッグするか、上のファイルパスを OBS の「ローカルファイル」に指定してください。', '브라우저로 패널을 열고 있어 드래그되는 것은 URL(현재 포트 고정)입니다. 포트에 묶이지 않는 버전은 데스크톱 앱에서 드래그하거나 위 파일 경로를 OBS "로컬 파일"에 지정하세요.', '目前是用浏览器打开面板，拖出去的是网址（会绑目前的 port）；要不绑 port 的版本请用桌面版拖，或复制上面的文件路径到 OBS“本地文件”。'],
    'system.portFallbackNotice': ['連接埠 {from} 被其他程式占用，這次改用 {to}。用「拖到 OBS」建立的來源不受影響；若 OBS 來源是貼網址建的，請把網址裡的 {from} 改成 {to}，或改用拖放重建。', 'Port {from} is in use by another app, so this session uses {to}. Sources created by dragging into OBS are unaffected; if you pasted a URL into OBS, change {from} to {to} in it or recreate the source by dragging.', 'ポート {from} が他のアプリに使われているため、今回は {to} を使用します。OBS へドラッグして作成したソースは影響を受けません。URL を貼り付けて作成した場合は、URL 内の {from} を {to} に変えるか、ドラッグで作り直してください。', '포트 {from}이(가) 다른 앱에서 사용 중이라 이번에는 {to}을(를) 사용합니다. OBS로 드래그해 만든 소스는 영향이 없습니다. URL을 붙여넣어 만든 소스라면 URL의 {from}을 {to}로 바꾸거나 드래그로 다시 만드세요.', '端口 {from} 被其他程序占用，这次改用 {to}。用“拖到 OBS”建立的来源不受影响；若 OBS 来源是粘贴网址建的，请把网址里的 {from} 改成 {to}，或改用拖放重建。'],
    'system.obsLauncherCopied': ['已複製檔案路徑', 'File path copied', 'ファイルパスをコピーしました', '파일 경로를 복사했습니다', '已复制文件路径'],
    'system.obsUrlHint': ['推薦用下面「拖到 OBS」建立來源。貼網址適合 OBS 在另一台電腦的情況；OBS WebSocket 連上後也可以一鍵建立。', 'Prefer "Drag into OBS" below. Pasting URLs suits OBS on another computer; with OBS WebSocket connected you can also create both sources in one click.', '下の「OBS にドラッグ」でのソース作成を推奨します。URL の貼り付けは OBS が別の PC にある場合向け。OBS WebSocket 接続時はワンクリック作成も可能です。', '아래 "OBS로 드래그"로 소스를 만드는 것을 권장합니다. URL 붙여넣기는 OBS가 다른 컴퓨터에 있을 때 적합하며, OBS WebSocket 연결 시 한 번에 만들 수도 있습니다.', '推荐用下面“拖到 OBS”建立来源。粘贴网址适合 OBS 在另一台电脑的情况；OBS WebSocket 连上后也可以一键建立。'],
    ...
    'controller.pairingNotice': ['請在桌面版 Elitesand Pro 產生配對 QR Code，再用手機掃描。PIN 若已啟用，配對後仍需要輸入 PIN。', "Generate a pairing QR code in the desktop Elitesand Pro app, then scan it with your phone. If a PIN is enabled, you'll still need to enter it after pairing.", 'デスクトップ版 Elitesand Pro でペアリング用 QR コードを表示し、スマートフォンでスキャンしてください。PIN が有効な場合、ペアリング後も PIN の入力が必要です。', '데스크톱 버전 Elitesand Pro에서 페어링용 QR 코드를 생성한 뒤 휴대폰으로 스캔하세요. PIN이 활성화되어 있으면 페어링 후에도 PIN을 입력해야 합니다.', '请在桌面版 Elitesand Pro 生成配对 QR Code，再用手机扫描。PIN 若已启用，配对后仍需要输入 PIN。'],
    ...
    'controller.collectionsHint': ['點一下選擇、再點一次就依順序加到播放清單末端。音檔已不在的歌要回桌面面板重新下載。', 'Tap once to select, tap again to append it to the playlist in order. Songs without a local audio file must be re-downloaded from the desktop panel.', '1 回タップで選択、もう 1 回タップで順番どおりプレイリストの末尾に追加します。音声ファイルがない曲はデスクトップ側で再ダウンロードしてください。', '한 번 탭하면 선택, 다시 탭하면 순서대로 재생목록 끝에 추가됩니다. 오디오 파일이 없는 곡은 데스크톱 패널에서 다시 내려받아야 합니다.', '点一下选择、再点一次就依顺序加到播放列表末端。音频已不在的歌要回桌面面板重新下载。'],
    'controller.collectionArmed': ['再點一次載入「{name}」', 'Tap again to load "{name}"', 'もう一度タップで「{name}」を読み込む', '다시 탭하여 "{name}" 불러오기', '再点一次载入「{name}」'],
    'controller.collectionLoading': ['載入中…', 'Loading…', '読み込み中…', '불러오는 중…', '载入中…'],
    'controller.collectionLoaded': ['已加入 {added} 首', 'Added {added} songs', '{added} 曲を追加しました', '{added}곡을 추가했습니다', '已加入 {added} 首'],
    'controller.collectionLoadedPartial': ['已加入 {added} 首；{pending} 首音檔不在，請在桌面面板重新下載', 'Added {added} songs; {pending} have no local audio and must be re-downloaded from the desktop panel', '{added} 曲を追加しました。{pending} 曲は音声ファイルがないため、デスクトップ側で再ダウンロードしてください', '{added}곡을 추가했습니다. {pending}곡은 오디오 파일이 없어 데스크톱 패널에서 다시 내려받아야 합니다', '已加入 {added} 首；{pending} 首音频不在，请在桌面面板重新下载'],
    'controller.collectionNothing': ['這個收藏歌單沒有可直接加入的歌', 'No songs in this collection are ready to add', 'このコレクションにはすぐ追加できる曲がありません', '이 컬렉션에는 바로 추가할 수 있는 곡이 없습니다', '这个收藏歌单没有可直接加入的歌'],
    'controller.collectionOverflow': ['播放清單已滿，{overflow} 首沒有加入', 'The playlist is full; {overflow} songs were not added', 'プレイリストが満杯のため {overflow} 曲は追加されませんでした', '재생목록이 가득 차 {overflow}곡은 추가되지 않았습니다', '播放列表已满，{overflow} 首没有加入'],
    ...
    'particle.entranceLabel': ['逐字進場', 'Per-glyph entrance', '一字ずつの登場', '글자별 등장', '逐字进场'],
    'particle.entranceAuto': ['原樣', 'Native', 'そのまま', '기본', '原样'],
    'particle.entranceQuaddrift': ['四相漂字', 'Quad Drift', '四方から漂う', '사방에서 떠도는', '四相漂字'],
    'particle.entranceAutoTitle': ['原樣：風／雨／渦流／雙束四種聚散風格逐句輪替', 'Native: rotates through four gather/scatter styles (wind, rain, vortex, twin) line by line', 'そのまま：風・雨・渦・双流の4つの集散スタイルを行ごとに順に切り替え', '기본: 바람/비/소용돌이/쌍류의 네 가지 모임/흩어짐 스타일이 줄마다 순서대로 바뀝니다', '原样：风／雨／涡流／双束四种聚散风格逐句轮替'],
    'particle.entranceQuaddriftTitle': ['四相漂字：每個字從四角帶弧度漂入定點、入場後完全靜止；可調動畫強度；退場仍是粒子散開', 'Quad Drift: each character arcs in from one of the four corners to its resting spot and stays fully still afterward; animation intensity is adjustable; the exit still scatters into particles as usual', '四相漂字：各文字が四隅のいずれかから弧を描いて所定の位置に流れ込み、その後は完全に静止します。アニメーションの強さは調整可能。退場は従来どおり粒子が散っていきます', '사방 흐름: 각 글자가 네 모서리 중 한 곳에서 곡선을 그리며 제자리로 흘러들어와 이후 완전히 정지합니다. 애니메이션 강도는 조절 가능하며, 퇴장은 기존과 같이 입자로 흩어집니다', '四相漂字：每个字从四角带弧度漂入定点、入场后完全静止；可调动画强度；退场仍是粒子散开'],
    'particle.entranceHint': ['原樣＝風／雨／渦流／雙束四種聚散風格逐句輪替；四相漂字＝改成每個字從四角漂入定點後靜止（可調沉穩／標準／狂放強度）。兩種選擇的退場都是原本的粒子散開，不會另外換退場效果。', 'Native = rotates through the four gather/scatter styles line by line; Quad Drift = every character instead drifts in from the four corners and settles still (with adjustable calm/normal/wild intensity). Either choice still exits by scattering into particles as usual.', 'そのまま＝4つの集散スタイルを行ごとに順に切り替え。四相漂字＝すべての文字が四隅から流れ込んで静止します（落ち着き／標準／激しさの強度調整可）。どちらを選んでも退場は従来どおり粒子が散っていきます。', '기본 = 네 가지 모임/흩어짐 스타일이 줄마다 순서대로 바뀝니다. 사방 흐름 = 모든 글자가 네 모서리에서 흘러들어와 정지합니다(차분함/보통/격렬함 강도 조절 가능). 어느 쪽을 선택해도 퇴장은 기존과 같이 입자로 흩어집니다.', '原样＝风／雨／涡流／双束四种聚散风格逐句轮替；四相漂字＝改成每个字从四角漂入定点后静止（可调沉稳／标准／狂放强度）。两种选择的退场都是原本的粒子散开，不会另外换退场效果。'],
    ...
    'tour.advanced.step.obsDrag.title': ['拖到 OBS，一步完成', 'Drag into OBS — one step', 'OBS へドラッグ、これだけ', 'OBS로 드래그, 한 번에 끝', '拖到 OBS，一步完成'],
    'tour.advanced.step.obsDrag.body': ['「歌詞畫面」與「歌單畫面」兩個方塊都可以直接拖進 OBS 預覽區。建出來的來源指向本機檔案，那個檔案會自己找到 Elitesand Pro 在哪個連接埠。', 'Both the "Lyrics display" and "Setlist display" tiles can be dragged straight onto the OBS preview. The created source points at a local file that finds Elitesand Pro’s port by itself.', '「歌詞画面」「セットリスト画面」の両タイルを OBS のプレビューへ直接ドラッグできます。作成されたソースはローカルファイルを指し、そのファイルが自動で Elitesand Pro のポートを見つけます。', '"가사 화면"과 "세트리스트 화면" 타일 모두 OBS 미리보기로 바로 드래그할 수 있습니다. 생성된 소스는 로컬 파일을 가리키며, 그 파일이 Elitesand Pro의 포트를 스스로 찾습니다.', '“歌词画面”与“歌单画面”两个方块都可以直接拖进 OBS 预览区。建出来的来源指向本地文件，那个文件会自己找到 Elitesand Pro 在哪个端口。'],
    'tour.advanced.step.obsDrag.hint': ['OBS 不在同一台電腦時才需要下一步的貼網址方式。', 'The next step (pasting a URL) is only needed when OBS runs on another computer.', '次のステップ（URL の貼り付け）は OBS が別の PC にある場合だけ必要です。', '다음 단계(URL 붙여넣기)는 OBS가 다른 컴퓨터에 있을 때만 필요합니다.', 'OBS 不在同一台电脑时才需要下一步的粘贴网址方式。'],
    'tour.advanced.step.obsUrl.title': ['備案：手動貼網址', 'Fallback: paste the URL manually', '代替手段：URL を手動で貼り付け', '대안: URL 수동 붙여넣기', '备案：手动粘贴网址'],
    'tour.advanced.step.obsUrl.body': ['複製歌詞畫面的 /display 網址，在 OBS 新增「瀏覽器」來源後貼上。適合 OBS 在另一台電腦、或不方便拖放的情況；缺點是網址綁著連接埠，換了就要改。', 'Copy the /display URL and paste it into a new OBS Browser Source. Use this when OBS runs on another computer or dragging is inconvenient; the downside is the URL is bound to the port.', '/display の URL をコピーし、OBS で新しいブラウザソースに貼り付けます。OBS が別の PC にある場合やドラッグが難しい場合向けで、URL がポートに固定される点が欠点です。', '/display URL을 복사해 OBS 브라우저 소스에 붙여넣습니다. OBS가 다른 컴퓨터에 있거나 드래그가 불편할 때 사용하며, URL이 포트에 묶이는 단점이 있습니다.', '复制歌词画面的 /display 网址，在 OBS 新增“浏览器”来源后粘贴。适合 OBS 在另一台电脑、或不方便拖放的情况；缺点是网址绑着端口，换了就要改。'],
    ...
    'tour.advanced.step.obsAiSeparation.hint': ['分離很吃時間，開台前先做好；完成後不用再開模式，播到這首時會自動使用雙軌，人聲預設 20% 可在播放區調整。', 'Separation takes a while, so do it before going live. No mode needs to be enabled afterward: the song automatically uses both stems, with vocals starting at 20% and adjustable in the playback controls.', '分離には時間がかかるため、配信前に済ませておきましょう。完了後に別のモードをオンにする必要はなく、その曲は自動で2トラックを使い、ボーカルは20%から始まり再生コントロールで調整できます。', '분리에는 시간이 걸리니 방송 전에 미리 해 두세요. 완료 후 별도 모드를 켤 필요 없이 해당 곡은 자동으로 두 트랙을 사용하며, 보컬은 기본 20%에서 시작해 재생 컨트롤에서 조절할 수 있습니다.', '分离很耗时间，开播前先做好；完成后不用再开模式，播放到这首时会自动使用双轨，人声默认 20% 可在播放区调整。'],
    'tour.advanced.step.obsDualAudio.title': ['雙路音訊：觀眾只收伴奏、耳機保留導唱', 'Dual audio: instrumental to viewers, guide vocal in headphones', 'デュアル音声：視聴者には伴奏、ヘッドホンにはガイド', '이중 오디오: 시청자는 반주만, 헤드폰에는 가이드 보컬', '双路音频：观众只收伴奏、耳机保留导唱'],
    'tour.advanced.step.obsDualAudio.body': ['已分離歌曲會自動用伴奏＋人聲雙軌播放。要讓 OBS／觀眾只收伴奏、自己的耳機保留人聲導唱，請在這張「雙路音訊路由」卡選兩個輸出裝置並開啟路由。', 'Separated songs automatically use instrumental + vocal stems. To send only the instrumental to OBS/viewers while keeping the guide vocal in your headphones, choose two output devices in this Dual audio routing card and enable routing.', '分離済みの曲は自動で伴奏＋ボーカルの2トラックを使います。OBS／視聴者には伴奏だけ、自分のヘッドホンにはガイドボーカルを残すには、この「デュアル音声ルーティング」カードで2つの出力デバイスを選び、ルーティングを有効にします。', '분리된 곡은 자동으로 반주＋보컬 두 트랙을 사용합니다. OBS/시청자에게는 반주만 보내고 본인 헤드폰에는 가이드 보컬을 남기려면 이 ‘이중 오디오 라우팅’ 카드에서 두 출력 장치를 고르고 라우팅을 켜세요.', '已分离歌曲会自动用伴奏＋人声双轨播放。要让 OBS／观众只收伴奏、自己的耳机保留人声导唱，请在这张“双路音频路由”卡选择两个输出设备并开启路由。'],
    'tour.advanced.step.obsDualAudio.hint': ['新使用者人聲預設 20%；首頁播放區可再調高／調低。OBS 和 Windows 的取樣率都建議設成 44100Hz。', 'Vocals start at 20% for new users and can be raised or lowered from the home playback controls. Set both OBS and Windows audio to 44100Hz.', '新規ユーザーのボーカル音量は20%が初期値で、ホームの再生コントロールから上げ下げできます。OBSとWindowsの音声はどちらも44100Hzに設定してください。', '새 사용자의 보컬 음량은 기본 20%이며 홈 재생 컨트롤에서 올리거나 내릴 수 있습니다. OBS와 Windows 오디오를 모두 44100Hz로 설정하세요.', '新用户人声默认 20%；首页播放区可再调高／调低。OBS 和 Windows 的采样率都建议设成 44100Hz。'],
    ...
    'tour.step.obs.body': ['把「歌詞畫面」方塊拖進 OBS 的預覽畫面，OBS 會自動新增一個尺寸等於畫布的瀏覽器來源，之後換歌、換模板、甚至換連接埠都不用再動它。', 'Drag the "Lyrics display" tile onto the OBS preview. OBS adds a canvas-sized Browser Source automatically, and you never touch it again — not when changing songs, templates, or even the port.', '「歌詞画面」タイルを OBS のプレビューへドラッグすると、キャンバスサイズのブラウザソースが自動で追加されます。曲・テンプレート・ポートが変わっても、以後触る必要はありません。', '"가사 화면" 타일을 OBS 미리보기로 드래그하면 캔버스 크기의 브라우저 소스가 자동으로 추가됩니다. 곡, 템플릿, 심지어 포트가 바뀌어도 다시 손댈 필요가 없습니다.', '把“歌词画面”方块拖进 OBS 的预览画面，OBS 会自动新增一个尺寸等于画布的浏览器来源，之后换歌、换模板、甚至换端口都不用再动它。'],
    'tour.step.obs.hint': ['拖不進去？按「複製檔案路徑」，在 OBS 新增瀏覽器來源時勾「本機檔案」選它。用瀏覽器開面板的話，拖出去的是網址，效果一樣但會綁目前的連接埠。', 'Can’t drag? Press "Copy file path", then add a Browser Source in OBS with "Local file" checked. From a plain browser the drag carries a URL instead — same result, but bound to the current port.', 'ドラッグできない場合は「ファイルパスをコピー」を押し、OBS でブラウザソースを追加する際に「ローカルファイル」を選んでください。通常のブラウザからのドラッグは URL になり、現在のポートに固定されます。', '드래그가 안 되면 "파일 경로 복사"를 누르고 OBS 브라우저 소스 추가 시 "로컬 파일"을 체크해 선택하세요. 일반 브라우저에서 드래그하면 URL이 전달되어 현재 포트에 묶입니다.', '拖不进去？按“复制文件路径”，在 OBS 新增浏览器来源时勾“本地文件”选它。用浏览器打开面板的话，拖出去的是网址，效果一样但会绑目前的端口。'],
    ...
    // ─── 公開點歌頁（2026-09-14 新增）───
    'nav.publicRequest': ['公開點歌頁', 'Public Requests', '公開リクエスト', '공개 신청곡', '公开点歌页'],
    'publicRequest.statusTitle': ['公開點歌頁', 'Public Requests', '公開リクエスト', '공개 신청곡', '公开点歌页'],
    'publicRequest.statusHint': [
      '開啟後，觀眾可從外網打開分享連結，搜尋你目前公開的歌單並送出點歌；請求會先進下方待核准清單，你按下「加入」才會真的排進播放清單。每位觀眾 20 秒最多 1 筆，整個房間每分鐘最多 6 筆。',
      'Once enabled, viewers can search the public catalog and send requests; you approve each one before it enters the queue. Each viewer can submit once every 20 seconds, with 6 requests per room per minute.',
      '有効にすると、視聴者は公開カタログから曲を検索してリクエストできます。承認するまで再生リストには入りません。各視聴者は20秒に1件、部屋全体では1分に6件までです。',
      '켜면 시청자가 공개 목록에서 곡을 검색해 신청할 수 있습니다. 승인하기 전에는 대기열에 들어가지 않습니다. 시청자당 20초에 1건, 방 전체는 분당 6건까지입니다.',
      '开启后，观众可从外网打开分享链接，搜索你目前公开的歌单并送出点歌；请求会先进下方待核准清单，你按下“加入”才会真的排进播放列表。每位观众 20 秒最多 1 笔，整个房间每分钟最多 6 笔。'
    ],
    'publicRequest.statusUnconfigured': ['尚未啟用', 'Not set up', '未設定', '미설정', '尚未启用'],
    'publicRequest.statusDisabled': ['已停用', 'Disabled', '無効', '비활성화됨', '已停用'],
    'publicRequest.statusConnected': ['已連線', 'Connected', '接続済み', '연결됨', '已连接'],
    'publicRequest.statusConnecting': ['連線中…', 'Connecting...', '接続中...', '연결 중...', '连接中…'],
    'publicRequest.statusReconnecting': ['重新連線中…', 'Reconnecting…', '再接続中…', '다시 연결 중…', '正在重新连接…'],
    'publicRequest.statusReplaced': ['已由其他程式接手', 'Another app instance took over', '別のアプリが引き継ぎました', '다른 앱 인스턴스가 인계했습니다', '已由其他程序接手'],
    'publicRequest.qrAlt': ['公開點歌頁 QR Code', 'Public request page QR code', '公開リクエストページの QR コード', '공개 신청곡 페이지 QR 코드', '公开点歌页二维码'],
    'publicRequest.shareLinkLabel': ['分享連結', 'Share link', '共有リンク', '공유 링크', '分享链接'],
    'publicRequest.copyLink': ['複製連結', 'Copy link', 'リンクをコピー', '링크 복사', '复制链接'],
    'publicRequest.catalogLabel': ['公開哪一份歌單', 'Playlist to publish', '公開する歌単を選択', '공개할 재생목록', '公开哪一份歌单'],
    'publicRequest.catalogNone': ['（尚未選擇）', '(Not selected)', '（未選択）', '(선택 안 함)', '（尚未选择）'],
    'publicRequest.catalogEmptyHint': [
      '尚未建立收藏歌單；也可以直接選擇「公開全部歌曲」，或用目前播放清單建立一份收藏歌單。',
      'No saved playlist yet. You can publish all songs or create one from the current playback queue.',
      '保存済みプレイリストがありません。「すべての曲を公開」するか、現在の再生リストから作成できます。',
      '저장된 재생목록이 없습니다. 전체 곡을 공개하거나 현재 재생목록으로 만들 수 있습니다.',
      '尚未建立收藏歌单。可以直接选择“公开全部歌曲”，或用当前播放列表建立一份收藏歌单。'
    ],
    'publicRequest.catalogAll': ['公開全部歌曲', 'Publish all songs', 'すべての曲を公開', '전체 곡 공개', '公开全部歌曲'],
    'publicRequest.createCatalogFromQueue': [
      '用目前播放清單建立公開歌單',
      'Create a public playlist from the current queue',
      '現在の再生リストから公開用プレイリストを作成',
      '현재 재생목록으로 공개 재생목록 만들기',
      '用当前播放列表建立公开歌单'
    ],
    'publicRequest.catalogDefaultName': ['公開點歌歌單', 'Public song requests', '公開リクエスト', '공개 신청곡', '公开点歌歌单'],
    'publicRequest.catalogCreateEmpty': ['目前播放清單沒有歌曲，先加入歌曲再建立公開歌單。', 'The current queue is empty. Add a song before creating a public playlist.', '現在の再生リストが空です。曲を追加してから公開用プレイリストを作成してください。', '현재 재생목록이 비어 있습니다. 공개 재생목록을 만들기 전에 곡을 추가하세요.', '当前播放列表没有歌曲，请先添加歌曲再建立公开歌单。'],
    'publicRequest.catalogCreateFailed': ['建立公開歌單失敗', 'Could not create the public playlist.', '公開用プレイリストを作成できませんでした。', '공개 재생목록을 만들 수 없습니다.', '建立公开歌单失败'],
    'publicRequest.catalogCreated': ['已建立「{name}」，並已選為公開歌單', 'Created “{name}” and selected it as the public playlist.', '「{name}」を作成し、公開用プレイリストに選択しました。', '“{name}”을 만들고 공개 재생목록으로 선택했습니다.', '已建立“{name}”，并已选为公开歌单'],
    'publicRequest.rotateLink': ['重新產生連結（舊連結立即失效）', 'Regenerate link (old link stops working immediately)', 'リンクを再生成（旧リンクは即座に無効）', '링크 재생성(기존 링크는 즉시 무효화됨)', '重新生成链接（旧链接立即失效）'],
    'publicRequest.pendingTitle': ['待核准點歌', 'Pending requests', '承認待ちリクエスト', '승인 대기 신청곡', '待核准点歌'],
    'publicRequest.pendingHint': [
      '觀眾的點歌會先排在這裡；你按「加入」才會真的排進播放清單，觀眾不會看到核准結果。',
      'Viewer requests land here first; they only enter the queue after you click Add, and viewers never see the approval result.',
      '視聴者のリクエストはまずここに並びます。「追加」を押すまで再生リストには入らず、視聴者には承認結果が表示されません。',
      '시청자의 신청곡은 먼저 여기에 표시됩니다. \'추가\'를 눌러야 실제로 재생목록에 들어가며, 시청자에게는 승인 결과가 표시되지 않습니다.',
      '观众的点歌会先排在这里；你按“加入”才会真的排进播放清单，观众不会看到核准结果。'
    ],
    'publicRequest.pendingEmpty': ['目前沒有待核准的點歌', 'No pending requests right now', '承認待ちのリクエストはありません', '현재 승인 대기 중인 신청곡이 없습니다', '目前没有待核准的点歌'],
    'publicRequest.requestedBy': ['{name} 點的', 'Requested by {name}', '{name} さんのリクエスト', '{name}님이 신청', '{name} 点的'],
    'publicRequest.reject': ['拒絕', 'Reject', '拒否', '거절', '拒绝'],
    'publicRequest.approveEnd': ['加入歌單尾端', 'Add to end of queue', 'リストの最後に追加', '목록 끝에 추가', '加入歌单尾端'],
    'publicRequest.approveNext': ['加入下一首', 'Play next', '次に再生', '다음 곡으로 추가', '加入下一首'],
    'publicRequest.approveFailed': ['加入播放清單失敗', 'Could not add to the playlist', '再生リストへの追加に失敗しました', '재생목록에 추가하지 못했습니다', '加入播放清单失败'],
    // 跟 i18n-auto.js 裡同一句 zh-TW 來源字串「已加入播放清單」共用同一組翻譯，
    // 避免重複鍵翻譯不一致（這個專案已經踩過一次「連線中…」/「已停用」同類的坑）。
    'publicRequest.approved': ['已加入播放清單', 'Added to playlist', 'プレイリストに追加しました', '재생목록에 추가했습니다', '已加入播放列表'],
    'publicRequest.rejectFailed': ['拒絕失敗', 'Could not reject the request', '拒否できませんでした', '거절하지 못했습니다', '拒绝失败'],
    'publicRequest.toggleFailed': ['切換公開點歌頁狀態失敗', 'Could not change the public request page status', '公開リクエストページの状態を切り替えられませんでした', '공개 신청곡 페이지 상태를 변경하지 못했습니다', '切换公开点歌页状态失败'],
    'publicRequest.rotateFailed': ['重新產生連結失敗', 'Could not regenerate the link', 'リンクを再生成できませんでした', '링크를 재생성하지 못했습니다', '重新生成链接失败'],
    'publicRequest.rotated': ['已產生新連結', 'New link generated', '新しいリンクを生成しました', '새 링크를 생성했습니다', '已生成新链接'],
    'publicRequest.linkCopied': ['已複製連結', 'Link copied', 'リンクをコピーしました', '링크를 복사했습니다', '已复制链接'],
    ...
    'lyricStyle.pickPerformance': ["先選擇演出畫面","Choose a stage layout first","まず演目の画面を選んでください","먼저 공연 화면을 선택하세요","先选择演出画面"],
    'lyricStyle.pickPerformanceHint': ["每個模板保留獨立外觀；切回來不會覆蓋先前調整。","Each template retains its independent appearance; switching back will not overwrite previous adjustments.","各テンプレートは独立した外観を保持します。元に戻しても、以前の調整は上書きされません。","각 템플릿은 독립적인 모양을 유지합니다. 다시 전환해도 이전 조정 내용을 덮어쓰지 않습니다.","每个模板保留独立外观；切回来不会覆盖先前调整。"],
    'template.classicDesc': ["完整歌詞與拼音／諧音","Full lyrics with romanization / phonetic guide","歌詞全文とローマ字・発音ガイド","가사 전문과 로마자 / 발음 가이드","完整歌词与拼音／谐音"],
    'template.pulseDesc': ["柔和流動、畫面中央","Soft flow, center of the picture","ソフトな流れ、画面中央","부드러운 흐름, 화면 가운데","柔和流动、画面中央"],
    'template.facetDesc': ["節點感強的舞台效果","Node-focused stage effect","ノード感の強いステージエフェクト","노드가 두드러지는 무대 효과","节点感强的舞台效果"],
    'template.driftDesc': ["斜向節拍、逐字律動","Offbeat rhythm, word-by-word motion","オフビートのリズム、単語単位の動き","엇박 리듬, 단어 단위 움직임","斜向节拍、逐字律动"],
    'template.auraDesc': ["大字留白、情緒演出","Large type, generous whitespace, emotive staging","大きな文字と余白、情緒的な演出","큰 글자와 여백, 감성적인 연출","大字留白、情绪演出"],
    'template.classicTitle': ["疊層顯示：歷史行淡出在上方，正在唱的一行在下方","Overlay display: the history line fades out at the top, and the current line is at the bottom","オーバーレイ表示: 履歴行が上部でフェードアウトし、現在の行が下部になります。","오버레이 표시: 히스토리 라인이 상단에서 페이드 아웃되고 현재 라인이 하단에 표시됩니다.","叠层显示：历史行淡出在上方，正在唱的一行在下方"],
    'template.pulseTitle': ["星砂流光：以星砂軌跡串起漂浮文字","Stardust Pulse: floating text linked by stardust trails","星砂の流光：星砂の軌跡でつながる浮遊テキスト","스타더스트 펄스: 별모래 궤적으로 이어지는 떠다니는 문자","星砂流光：以星砂轨迹串起漂浮文字"],
    'template.facetTitle': ["折光階梯：虛線光尺與切角節點分段進場","Prism Steps: dashed light rulers and bevelled nodes enter in stages","屈折の階段：破線の光スケールと切り欠きノードが段階的に登場","굴절 계단: 점선 광선 눈금자와 모따기 노드가 단계적으로 등장","折光阶梯：虚线光尺与切角节点分段进场"],
    'template.driftTitle': ["斜拍告白：斜向節拍與逐字律動","Offbeat Confession: offbeat rhythm with word-by-word motion","オフビートの告白：オフビートのリズムと単語単位の動き","엇박 고백: 엇박 리듬과 단어 단위 움직임","斜拍告白：斜向节拍与逐字律动"],
    'template.auraTitle': ["潮汐心景：呼吸光環中的空間文字","Tidal Mindscape: spatial text inside a breathing halo","潮汐の心景：呼吸する光輪の中の空間文字","조수 심상: 숨 쉬는 광륜 속 공간 문자","潮汐心景：呼吸光环中的空间文字"],
    'lyricStyle.lyricPosition': ["歌詞位置","Lyrics position","歌詞の位置","가사 위치","歌词位置"],
    'lyricStyle.posCenterTitle': ["置中：歌詞在畫面正中央（預設）","Centered: The lyrics are in the center of the screen (default)","Centered: 歌詞が画面の中央に表示されます (デフォルト)。","중앙: 가사가 화면 중앙에 위치함(기본값)","置中：歌词在画面正中央（预设）"],
    'lyricStyle.posLeftTitle': ["偏左：歌詞集中在畫面左半，右邊留給人物","Left: The lyrics are concentrated on the left half of the screen, leaving the characters on the right","左：歌詞が画面の左半分に集中し、右側は配信者の立ち絵用に空きます","왼쪽: 화면 왼쪽에 가사가 집중되어 있고 오른쪽은 방송인 캐릭터 자리로 비워 둡니다.","偏左：歌词集中在画面左半，右边留给人物"],
    'lyricStyle.posRightTitle': ["偏右：歌詞集中在畫面右半，左邊留給人物","Right: The lyrics are concentrated on the right half of the screen, leaving the left side for the characters","右: 歌詞が画面の右半分に集中し、左側は配信者の立ち絵用に空きます。","오른쪽: 가사가 화면 오른쪽에 집중되어 있고 왼쪽에는 캐릭터가 표시됩니다.","偏右：歌词集中在画面右半，左边留给人物"],
    'lyricStyle.posSplitTitle': ["左右分散：一句左、一句右交替出現，中間留給人物（經典疊層不支援）","Split left/right: lines alternate between the left and right sides, leaving the center clear for the character. (Not supported by Classic Overlay.)","左右分散: 中央を配信者の立ち絵用に空けたまま、左の文と右の文が交互に表示されます (クラシック オーバーレイはサポートされていません)","좌우 분산: 가사 줄이 왼쪽과 오른쪽에 번갈아 나타나며 가운데는 캐릭터 자리로 비워 둡니다(클래식 오버레이에서는 지원하지 않음).","左右分散：一句左、一句右交替出现，中间留给人物（经典叠层不支持）"],
    'lyricStyle.resetPosition': ["重置位置","Reset position","位置をリセットする","위치 재설정","重置位置"],
    'lyricStyle.dragGridHint': ["拖曳選格子，或用下面細調微調","Drag to pick a cell, or use the fine controls below","ドラッグしてマス目を選ぶか、下の細かい調整で微調整します","드래그하여 그리드를 선택하거나 아래의 미세 조정을 사용하여 미세 조정하세요.","拖动选格子，或用下面细调微调"],
    'lyricStyle.posTopLeft': ["上 左","Upper left","左上","왼쪽 위","上 左"],
    'lyricStyle.posTopCenter': ["上 中","Upper middle","上中","중간 상단","上 中"],
    'lyricStyle.posTopRight': ["上 右","Upper right","右上","오른쪽 위","上 右"],
    'lyricStyle.posMidLeft': ["中 左","Center left","中央左","중앙 왼쪽","中 左"],
    'lyricStyle.posCenterCenter': ["正中","Center","中央","가운데","正中"],
    'lyricStyle.posMidRight': ["中 右","Center right","中央右","중앙 오른쪽","中 右"],
    'lyricStyle.posBottomLeft': ["下 左","Bottom left","左下","왼쪽 아래","下 左"],
    'lyricStyle.posBottomCenter': ["下 中","Lower middle","中下","중간 하단","下 中"],
    'lyricStyle.posBottomRight': ["下 右","Bottom right","右下","오른쪽 아래","下 右"],
    'lyricStyle.posHint': ["方便 VTuber／實況主把人物放在畫面固定位置；「左右分散」是一句左、一句右交替。","Useful for VTubers and streamers who keep their character in a fixed screen position. \"Split left/right\" alternates lyric lines between the two sides.","VTuber や配信者がキャラクターを画面上の固定位置に置くのに便利です。「左右分散」は、左の行と右の行を交互に表示します。","VTuber와 스트리머가 캐릭터를 화면의 고정된 위치에 둘 때 유용합니다. \"좌우 분산\"은 가사 줄을 왼쪽과 오른쪽에 번갈아 표시합니다.","方便 VTuber／主播把人物放在画面固定位置；“左右分散”是一句左、一句右交替。"],
    'lyricStyle.resetFineTune': ["重置微調","Reset fine-tuning","微調整をリセット","트림 재설정","重置微调"],
    'lyricStyle.fineTuneHint': ["小範圍微調，超出安全範圍會自動收回","Small-range fine-tuning; anything beyond the safe range snaps back automatically","小範囲の微調整です。安全範囲を超えると自動的に戻ります","작은 범위의 미세 조정입니다. 안전 범위를 벗어나면 자동으로 되돌아갑니다","小范围微调，超出安全范围会自动收回"],
    'lyricStyle.horizontalX': ["水平 X","Horizontal X","レベルX","레벨 X","水平 X"],
    'lyricStyle.verticalY': ["垂直 Y","Vertical Y","垂直Y","수직 Y","垂直 Y"],
    'lyricStyle.intensityTitle': ["動畫強度","Animation strength","アニメーションの強さ","애니메이션 강도","动画强度"],
    'lyricStyle.intensityCalmTitle': ["沉穩：置中排列、無散射旋轉","Calm: centered layout, no scatter or rotation","落ち着き：中央揃え、散乱や回転なし","차분함: 가운데 정렬, 흩어짐과 회전 없음","沉稳：居中排列、无散射旋转"],
    'lyricStyle.intensityNormalTitle': ["標準：適度散射與旋轉","Standard: Moderate scattering and rotation","標準: 中程度の散乱と回転","표준: 적당한 산란 및 회전","标准：适度散射与旋转"],
    'lyricStyle.intensityChaoticTitle': ["狂放：大幅散射、隨機對齊","Wild: large scattering, random alignment","ワイルド: 大きな散乱、ランダムな配置","과감함: 크게 흩어지고 무작위로 정렬","狂放：大幅散射、随机对齐"],
    'lyricStyle.intensityCalm': ["沉穩","Calm","穏やかな","진정","沉稳"],
    'lyricStyle.intensityNormal': ["標準","Standard","標準","표준","标准"],
    'lyricStyle.intensityChaotic': ["狂放","Bold","大胆","대담","狂放"],
    'lyricStyle.saveAppearance': ["儲存外觀","Save appearance","外観の保存","모양 저장","保存外观"],
    'lyricStyle.saveAppearanceHint': ["保存目前模板的字體、顏色與位置，快速套用到下一次演出","Save the font, color and position of the current template and quickly apply it to the next performance","現在のテンプレートのフォント、色、位置を保存し、次のパフォーマンスにすぐに適用します","현재 템플릿의 글꼴, 색상, 위치를 저장하고 다음 연주에 빠르게 적용하세요.","保存目前模板的字体、颜色与位置，快速应用到下一次演出"],
    'lyricStyle.presetAriaLabel': ["歌詞外觀預設","Lyrics appearance preset","歌詞表示プリセット","가사 모양 프리셋","歌词外观预设"],
    'lyricStyle.presetNone': ["尚未保存預設","No saved presets","保存済みプリセットなし","저장된 프리셋 없음","尚未保存预设"],
    'lyricStyle.presetSave': ["保存預設","Save preset","プリセットを保存","프리셋 저장","保存预设"],
    'common.delete': ["刪除","Delete","削除","삭제","删除"],
    'lyricStyle.styleCuteTitle': ["Q 彈：字像果凍一樣跳進來，粉紅光暈＋粒子","Bouncy: words pop in like jelly, with a pink halo and particles","バウンシー：文字がゼリーのように弾んで現れ、ピンクの光輪と粒子を伴います。","탱글탱글: 글자가 젤리처럼 튀어 들어오고, 분홍 후광과 입자가 함께합니다.","Q 弹：字像果冻一样跳进来，粉红光晕＋粒子"],
    'lyricStyle.styleRockTitle': ["砸落：整行猛力砸進畫面，紅色烈焰光","Smash: The whole row slams into the screen with red flames.","打ち付け：行全体が勢いよく画面に叩きつけられ、赤い炎が走ります","스매시(Smash): 전체 행이 붉은 불꽃과 함께 화면에 부딪칩니다.","砸落：整行猛力砸进画面，红色烈焰光"],
    'lyricStyle.styleBalladTitle': ["漂浮：緩慢淡入、逐字如水波浮現，藍色月光","Floating: slowly fading in, word-by-word emerging like water waves, blue moonlight","フローティング：ゆっくりフェードインし、文字が水波のように一文字ずつ現れる。青い月光。","부유: 천천히 페이드 인되며, 글자가 물결처럼 하나씩 나타나고 푸른 달빛이 더해집니다.","漂浮：缓慢淡入、逐字如水波浮现，蓝色月光"],
    'lyricStyle.styleSparkleTitle': ["綻放：字由小點綻放開來，金粉光暈＋粒子","Bloom: the words bloom from small dots, golden halo + particles","ブルーム: 小さな点、金色の後光 + 粒子から言葉が咲きます","블룸: 작은 점에서 글자가 피어나며, 황금빛 후광과 입자가 함께 나타납니다.","绽放：字由小点绽放开来，金粉光晕＋粒子"],
    'lyricStyle.styleDreamyTitle': ["聚焦：從模糊鬼影慢慢聚焦成形，紫色柔光","Focus: gradually resolves from a blurred ghost image, with a soft purple glow.","フォーカス：ぼやけた残像がゆっくり焦点を結び、紫の柔らかな光を帯びます。","초점: 흐릿한 잔상이 천천히 또렷해지고 보랏빛의 부드러운 빛이 더해집니다.","聚焦：从模糊鬼影慢慢聚焦成形，紫色柔光"],
    'lyricStyle.styleMinimalTitle': ["字幕感：純淡入、零位移零光效，最安靜","Subtitle-like: pure fade-in, no movement, no glow — the quietest option","字幕風：フェードインのみ。移動も発光もなく、最も静かです","자막 느낌: 순수한 페이드인, 제로 변위 및 제로 조명 효과, 가장 조용함","字幕感：纯淡入、零位移零光效，最安静"],
    'lyricStyle.styleCute': ["可愛","Cute","かわいい","귀여움","可爱"],
    'lyricStyle.styleRock': ["搖滾","Rock","ロック","록","摇滚"],
    'lyricStyle.styleBallad': ["抒情","Ballad","バラード","발라드","抒情"],
    'lyricStyle.styleSparkle': ["夢幻","Dreamy","夢のような","꿈꾸는 듯한","梦幻"],
    'lyricStyle.styleDreamy': ["空靈","Ethereal","幽玄","몽환","空灵"],
    'lyricStyle.styleMinimal': ["簡約","Simple","シンプル","단순","简约"],
    'lyricStyle.styleDescHint': ["每張縮圖的「字」會用該風格的高亮動態跑一次；滑鼠停在上面可看文字說明。","Each thumbnail previews text once with that style’s highlight animation; hover to read its description.","各サムネイルの「文字」が、そのスタイルのハイライト演出で 1 回再生されます。マウスを重ねると説明を確認できます。","각 미리보기의 \"단어\"는 해당 스타일의 하이라이트 애니메이션으로 한 번 재생됩니다. 항목 위에 마우스를 올리면 텍스트 설명을 볼 수 있습니다.","每张缩略图的“字”会用该风格的高亮动态跑一次；鼠标停在上面可看文字说明。"],
    'lyricStyle.styleTuneSummary': ["風格微調","Style fine-tuning","スタイルの微調整","스타일 미세 조정","风格微调"],
    'lyricStyle.animSpeedTitle': ["整體動畫快慢：小於 1 變慢、大於 1 變快","Overall animation speed: below 1 is slower; above 1 is faster.","全体的なアニメーション速度: 1 未満の場合は遅くなり、1 を超える場合は速くなります","전체 애니메이션 속도: 1보다 작으면 느려지고, 1보다 크면 빨라집니다.","整体动画快慢：小于 1 变慢、大于 1 变快"],
    'lyricStyle.animSpeedLabel': ["動畫速度","Animation speed","アニメーションの速度","애니메이션 속도","动画速度"],
    'lyricStyle.highlightScaleTitle': ["唱到的字被放大的倍率","How much the character being sung is enlarged","歌っている文字の拡大倍率","노래된 단어의 확대 비율","唱到的字被放大的倍率"],
    'lyricStyle.highlightScaleLabel': ["高亮放大（唱到的字）","Highlight and enlarge (the character being sung)","ハイライト拡大（歌っている文字）","강조 표시 및 확대(가사)","高亮放大（唱到的字）"],
    'lyricStyle.entryOffsetTitle': ["新的一行從多遠的下方進場，0＝原地淡入","Entry distance from below; 0 = fade in in place.","新しい行が下から入場する距離。0＝その場でフェードイン","새 줄이 아래에서 얼마나 멀리 등장하는지. 0이면 제자리에서 페이드 인합니다.","新的一行从多远的下方进场，0＝原地淡入"],
    'lyricStyle.entryOffsetLabel': ["入場位移（進場距離）","Entry offset (travel distance)","登場オフセット（移動距離）","등장 오프셋(이동 거리)","入场位移（进场距离）"],
    'lyricStyle.entryBlurTitle': ["進場瞬間的模糊量，越大越夢幻","Blur at the moment of entry — higher is dreamier","登場した瞬間のぼかし量。大きいほど幻想的になります","등장 순간의 블러 양 — 클수록 몽환적입니다","进场瞬间的模糊量，越大越梦幻"],
    'lyricStyle.entryBlurLabel': ["入場模糊","Entry blur","登場ぼかし","등장 블러","入场模糊"],
    'lyricStyle.staggerTitle': ["一行裡每個字進場的時間差，0＝整行同時出現","Delay between characters entering a line; 0 = the entire line appears at once.","1 行内の各文字が現れる時間差。0＝行全体が同時に表示されます。","한 줄 안에서 각 글자가 등장하는 시간 차이. 0이면 줄 전체가 동시에 나타납니다.","一行里每个字进场的时间差，0＝整行同时出现"],
    'lyricStyle.staggerLabel': ["字間錯落（逐字時間差）","Character stagger (per-character delay)","文字のずれ（単語ごとの時間差）","글자 엇갈림(단어별 시간차)","字间错落（逐字时间差）"],
    'lyricStyle.particlesLabel': ["粒子效果","Particle effects","パーティクルエフェクト","입자 효과","粒子效果"],
    'lyricStyle.particlesHint': ["關閉可減輕負載／更乾淨","Turning this off reduces load and gives a cleaner look.","オフにすると負荷が軽減され、表示がすっきりします","끄면 부하가 줄고 화면이 더 깔끔해집니다","关闭可减轻负载／更干净"],
    'lyricStyle.styleTuneNote': ["微調會即時套用到 OBS 並隨風格切換重置。","Fine-tuning is instantly applied to OBS and resets when switching styles.","微調整は即座に OBS に適用され、スタイルを切り替えるとリセットされます。","미세 조정은 OBS에 즉시 적용되며 스타일 전환 시 재설정됩니다.","微调会即时应用到 OBS 并随风格切换重置。"],
    'lyricStyle.fontColorTitle': ["字體與顏色","Fonts and colors","フォントと色","글꼴 및 색상","字体与颜色"],
    'lyricStyle.lyricColorLabel': ["歌詞文字顏色","Lyric text color","歌詞の文字色","가사 텍스트 색상","歌词文字颜色"],
    'lyricStyle.activeLineColorLabel': ["「正在唱那一行」的顏色","Color of the line being sung","「今歌っている行」の色","‘지금 부르는 줄’의 색상","“正在唱那一行”的颜色"],
    'lyricStyle.activeColorHint': ["逐字 KTV 模式下「唱到的字」也是用右邊「正在唱那一行的顏色」；發光效果則在下面「進階效果」裡的「額外發光」統一設定，兩種模式共用。","In the word-by-word KTV mode, the \"words sung\" also use the \"color of the line being sung\" on the right; the glow effect is set uniformly in the \"Extra Glow\" in the \"Advanced Effects\" below, and is shared by both modes.","単語単位の KTV モードでは、「歌唱済みの文字」にも右側の「歌唱中の行の色」を使います。グローは下の「高度なエフェクト」の「追加グロー」で一括設定され、両モードで共通です。","단어 단위 KTV 모드에서는 \"노래하는 단어\"도 오른쪽의 \"노래되는 줄의 색상\"을 사용합니다. 글로우 효과는 아래 \"고급 효과\"의 \"추가 글로우\"에서 균일하게 설정되며 두 모드에서 공유됩니다.","逐字 KTV 模式下“唱到的字”也是用右边“正在唱那一行的颜色”；发光效果则在下面“进阶效果”里的“额外发光”统一设置，两种模式共用。"],
    'lyricStyle.fontFamilyLabel': ["字體","Font","フォント","글꼴","字体"],
    'font.notoSans': ["Noto Sans（黑體）","Noto Sans (sans serif)","Noto Sans (太字ボディ)","Noto Sans(고딕체)","Noto Sans（黑体）"],
    'font.notoSerif': ["Noto Serif（明體）","Noto Serif (serif)","Noto Serif（明朝体）","Noto Serif(명조체)","Noto Serif（明体）"],
    'font.systemFont': ["系統字體","System font","システムフォント","시스템 글꼴","系统字体"],
    'font.monospaceFont': ["等寬字體","Monospaced font","等幅フォント","고정 폭 글꼴","等宽字体"],
    'font.customFont': ["自訂／系統字體…","Custom / system font…","カスタム／システムフォント…","사용자 지정/시스템 글꼴…","自定义／系统字体…"],
    'font.loadSystemFontsPlaceholder': ["點右側「瀏覽系統字體」載入清單…","Click \"Browse System Fonts\" on the right to load the list...","右側の「システムフォントの参照」をクリックしてリストをロードします...","목록을 로드하려면 오른쪽의 \"시스템 글꼴 찾아보기\"를 클릭하세요.","点右侧“浏览系统字体”加载列表…"],
    'font.browseSystemFonts': ["瀏覽系統字體","Browse system fonts","システムフォントを参照する","시스템 글꼴 찾아보기","浏览系统字体"],
    'font.customFontPlaceholder': ["或手動輸入字體名稱，例如：微軟正黑體","Or enter a font name manually, for example: Segoe UI.","またはフォント名を手動で入力します（例：メイリオ）。","또는 글꼴 이름을 직접 입력하세요(예: 맑은 고딕).","或手动输入字体名称，例如：微软雅黑"],
    'lyricStyle.latinFontLabel': ["英文／數字字體（選填）","English/numeric font (optional)","英語/数字フォント (オプション)","영문/숫자 폰트(선택)","英文／数字字体（选填）"],
    'font.latinFontNone': ["（不指定：英文跟主字體）","(Unspecified: English follows the main font)","（指定なし：英語とメインフォント）","(지정 안 함: 영문도 주 글꼴을 따름)","（不指定：英文跟主字体）"],
    'font.latinFontPlaceholder': ["或手動輸入英文字體名稱，例如：Montserrat","Or manually enter the English font name, for example: Montserrat","または、英語のフォント名を手動で入力します (例: Montserrat)。","또는 영어 글꼴 이름을 수동으로 입력하세요(예: Montserrat).","或手动输入英文字体名称，例如：Montserrat"],
    'lyricStyle.latinFontHint': ["讓英文字母和數字永遠用另一套字體（例如中文用圓體、英文用 Montserrat）。中日韓文字不受影響，仍用上面的主字體。","Let English letters and numbers always use another set of fonts (for example, Chinese uses Yuanli, and English uses Montserrat). Chinese, Japanese and Korean characters are not affected and still use the main font above.","英語の文字と数字には常に別のフォント セットを使用します (たとえば、中国語では Yuanli を使用し、英語では Montserrat を使用します)。中国語、日本語、韓国語の文字は影響を受けず、引き続き上記のメイン フォントを使用します。","영어 문자와 숫자는 항상 다른 글꼴 세트를 사용하도록 합니다. 예를 들어 중국어는 Yuanli를 사용하고 영어는 Montserrat를 사용합니다. 중국어, 일본어, 한국어 문자는 영향을 받지 않으며 여전히 위의 기본 글꼴을 사용합니다.","让英文字母和数字永远用另一套字体（例如中文用圆体、英文用 Montserrat）。中日韩文字不受影响，仍用上面的主字体。"],
    'font.weightLabel': ["字重", "Font weight", "フォントの太さ", "글꼴 두께", "字重"],
    'font.weightNormal': ["一般","Regular","標準","보통","常规"],
    'font.weightBold': ["粗體","Bold","太字","굵게","粗体"],
    'font.weightBlack': ["極粗","Extra bold","極厚","매우 굵게","极粗"],
    'lyricStyle.romajiXieyinTitle': ["羅馬字與諧音","Romanization and phonetic guide","ローマ字と発音ガイド","로마자와 발음 가이드","罗马字与谐音"],
    'lyricStyle.romajiXieyinIntro': ["調整拼音（羅馬字）與中文諧音行的顏色與大小。字級為相對主字級的比例。","Adjust the color and size of the romanization and Chinese phonetic-guide lines. Font size is relative to the main lyric font size.","ローマ字表記と中国語の発音ガイド行の色とサイズを調整します。フォントサイズはメイン歌詞の文字サイズに対する比率です。","중국어 로마자 표기와 중국어 음차 가사 줄의 색상과 크기를 조정합니다. 글꼴 크기는 기본 가사 글꼴 크기에 대한 비율입니다.","调整罗马字与中文谐音行的颜色和大小。字号按主歌词字号的比例计算。"],
    'lyricStyle.romajiSizeLabel': ["羅馬字大小","Romanization size","ローマ字フォントサイズ","로마자 글꼴 크기","罗马字大小"],
    'lyricStyle.romajiColorLabel': ["羅馬字顏色","Romanization color","ローマ字カラー","로마자 색상","罗马字颜色"],
    'lyricStyle.xieyinSizeLabel': ["諧音大小","Phonetic guide size","発音ガイドのサイズ","발음 가이드 크기","谐音大小"],
    'lyricStyle.xieyinColorLabel': ["諧音顏色","Phonetic guide color","発音ガイドの色","발음 가이드 색상","谐音颜色"],
    'display.layoutTitle': ["位置與排版","Position and layout","位置とレイアウト","위치 및 레이아웃","位置与排版"],
    'display.layoutMovedHint': ["歌詞在畫面上的九宮格定位已搬到「顯示設定」首頁「動畫風格」卡片裡的「歌詞位置」，選經典疊層時會出現。","The nine-square grid positioning of lyrics on the screen has been moved to the \"Lyrics Position\" in the \"Animation Style\" card on the \"Display Settings\" homepage, and will appear when selecting Classic Overlay.","画面上の歌詞の 9 マスのグリッドの位置は、「表示設定」ホームページの「アニメーション スタイル」カードの「歌詞の位置」に移動され、クラシック オーバーレイを選択すると表示されます。","화면에서 가사의 9칸 그리드 위치가 \"디스플레이 설정\" 홈페이지의 \"애니메이션 스타일\" 카드에 있는 \"가사 위치\"로 이동되었으며, 클래식 오버레이를 선택하면 표시됩니다.","歌词在画面上的九宫格定位已搬到“显示设置”首页“动画风格”卡片里的“歌词位置”，选经典叠层时会出现。"],
    'display.textAlignLabel': ["文字對齊","Text alignment","テキストの配置","텍스트 정렬","文字对齐"],
    'align.left': ["靠左","Align left","左揃え","왼쪽 정렬","靠左"],
    'align.right': ["靠右","Align right","右揃え","오른쪽 정렬","靠右"],
    'display.lineHeightLabel': ["行高","Row height","行の高さ","행 높이","行高"],
    'display.letterSpacingLabel': ["字距","Letter spacing","カーニング","커닝","字距"],
    'display.marginMaxWidthSummary': ["邊距 · 最大寬度","Margin · Maximum Width","余白・最大幅","여백 · 최대 너비","边距 · 最大宽度"],
    'display.paddingXLabel': ["左右邊距","Left/right padding","左右の余白","좌우 여백","左右边距"],
    'display.paddingYLabel': ["上下邊距","Top and bottom margins","上下の余白","위쪽 및 아래쪽 여백","上下边距"],
    'display.maxWidthLabel': ["歌詞區最大寬度","Maximum width of lyrics area","歌詞エリアの最大幅","가사 영역의 최대 너비","歌词区最大宽度"],
    'display.advancedEffectsTitle': ["進階效果","Advanced effects","高度なエフェクト","고급 효과","进阶效果"],
    'display.activeScaleLabel': ["當前行放大","Enlarge current line","現在の行を拡大する","현재 줄 확대","当前行放大"],
    'display.historyLinesLabel': ["保留上一句行數","Number of previous lines to keep","前の行を残す行数","이전 줄 유지 수","保留上一句行数"],
    'display.historyFontSizeLabel': ["歷史歌詞字級","Previous-lines font size","過去の歌詞のサイズ","지난 가사 크기","历史歌词字号"],
    'display.historyFontSizeHint': ["拉到最左＝自動（主字級的 58%）；往右可獨立指定大小","Drag all the way left for Auto (58% of the main font size); drag right to set the size independently.","左端＝自動（メインのフォントサイズの 58%）。右に動かすとサイズを個別に指定できます","맨 왼쪽으로 당기기 = 자동(기본 글꼴 크기의 58%); 독립적으로 크기를 지정하려면 오른쪽으로 이동하세요.","拉到最左＝自动（主字级的 58%）；往右可独立指定大小"],
    'display.historyOpacityLabel': ["歷史歌詞透明度","Previous-lines opacity","歴史的な歌詞の透明性","지난 가사 불투명도","历史歌词透明度"],
    'display.strokeShadowGlowBgSummary': ["描邊 · 陰影 · 發光 · 背景","Stroke · Shadow · Glow · Background","ストローク・シャドウ・グロー・背景","스트로크 · 그림자 · 빛 · 배경","描边 · 阴影 · 发光 · 背景"],
    'display.effectsLayersHint': ["這四種效果是獨立的四層，由上到下：描邊＝字的外框線、陰影＝字後面的影子、發光＝「正在唱那行」的光暈、背景框＝歌詞後面的色塊。另外「動畫風格」本身也自帶一層高亮光暈（那是風格的一部分，換風格會變）。","These are four independent layers: stroke = the text outline; shadow = the shadow behind text; glow = the halo on the current lyric line; and background frame = the color block behind lyrics. Animation style also has its own highlight halo, which changes with the selected style.","これら 4 つは独立したレイヤーです。上から順に、ストローク＝文字の輪郭線、シャドウ＝文字の後ろの影、グロー＝「歌唱中の行」の光輪、背景フレーム＝歌詞の後ろの色面です。さらに「アニメーションスタイル」自体にもハイライトの光輪があり、スタイルを変更すると一緒に変わります。","이 네 가지 효과는 서로 독립된 레이어입니다. 위에서부터 외곽선은 글자의 테두리, 그림자는 글자 뒤의 그림자, 글로우는 “현재 부르는 줄”을 나타내는 후광, 배경 상자는 가사 뒤의 색상 블록입니다. “애니메이션 스타일”에도 자체 강조 후광이 포함되어 있으며 스타일을 바꾸면 함께 변경됩니다.","这四种效果是独立的四层，由上到下：描边＝字的外框线、阴影＝字后面的影子、发光＝“正在唱那行”的光晕、背景框＝歌词后面的色块。另外“动画风格”本身也自带一层高亮光晕（那是风格的一部分，换风格会变）。"],
    'display.strokeWidthLabel': ["文字描邊寬度（所有歌詞行）","Text stroke width (all lyric lines)","テキストのストローク幅（すべての歌詞行）","텍스트 획 너비(모든 가사 행)","文字描边宽度（所有歌词行）"],
    'display.strokeColorLabel': ["描邊顏色","Stroke color","ストロークの色","획 색상","描边颜色"],
    'display.shadowLabel': ["文字陰影（所有歌詞行）","Text shadow (all lyric lines)","文字の影（すべての歌詞行）","텍스트 그림자(모든 가사 줄)","文字阴影（所有歌词行）"],
    'shadow.none': ["無","None","なし","없음","无"],
    'shadow.soft': ["柔和","Soft","ソフト","소프트","柔和"],
    'shadow.hard': ["硬陰影","Hard shadow","ハードシャドウ","단단한 그림자","硬阴影"],
    'shadow.glow': ["發光","Glow","輝き","글로우","发光"],
    'shadow.outline': ["外框","Outer frame","外枠","외부 프레임","外框"],
    'display.shadowColorLabel': ["陰影顏色","Shadow color","影の色","그림자 색상","阴影颜色"],
    'display.glowStrengthLabel': ["「正在唱那一行」的額外發光","Extra glow for the current lyric line","「歌唱中の行」の追加グロー","\"현재 부르는 줄\" 추가 글로우","“正在唱那一行”的额外发光"],
    'display.glowColorLabel': ["發光顏色","Glow color","発光の色","발광 색상","发光颜色"],
    'display.glowHint': ["逐句與逐字 KTV 模式都吃這個設定；強度拉到 0（關）時，逐字模式才會改用「動畫風格」自帶的光暈色。","Both line-by-line and word-by-word KTV modes use this setting; when intensity is set to 0 (off), word-by-word mode uses the glow color built into the animation style.","行単位・単語単位どちらの KTV モードでもこの設定が使われます。強度を 0（オフ）にすると、単語単位モードは「アニメーションスタイル」に付属する光輪の色に切り替わります。","줄 단위 및 단어 단위 KTV 모드는 모두 이 설정을 사용합니다. 강도를 0(끄기)으로 낮추면 단어 단위 모드는 “애니메이션 스타일”에 포함된 후광 색상을 사용합니다.","逐句与逐字 KTV 模式都吃这个设置；强度拉到 0（关）时，逐字模式才会改用“动画风格”自带的光晕色。"],
    'display.bgOpacityFieldLabel': ["歌詞背景框（半透明色塊）","Lyrics background panel (translucent color block)","歌詞背景フレーム（半透明カラーブロック）","가사 배경 프레임(반투명 컬러 블록)","歌词背景框（半透明色块）"],
    'display.bgColorLabel': ["背景顏色","Background color","背景色","배경색","背景颜色"],
    'display.backgroundTitle': ["背景","Background","背景","배경","背景"],
    'display.backgroundHint': ["背景會鋪滿整個 OBS 來源。維持無背景＝透明疊加（預設）。","The background fills the whole OBS source. Leave it on \"No background\" for a transparent overlay (default).","背景は OBS ソース全体を埋めます。背景なし = 透明なオーバーレイを維持します (デフォルト)。","배경은 전체 OBS 소스를 채웁니다. 배경 없음 = 투명 오버레이 유지(기본값)","背景会铺满整个 OBS 来源。维持无背景＝透明叠加（预设）。"],
    'display.currentBgPreviewAlt': ["目前背景預覽","Current background preview","現在の背景プレビュー","현재 배경 미리보기","目前背景预览"],
    'display.uploadBgImage': ["上傳背景圖","Upload background image","背景画像をアップロードする","배경 이미지 업로드","上传背景图"],
    'display.removeBackground': ["移除背景","Remove background","背景を削除する","배경 제거","移除背景"],
    'display.bgOpacityLabel': ["背景透明度","Background opacity","背景の透明度","배경 투명도","背景透明度"],
    'display.bgFitLabel': ["顯示方式","Display mode","表示モード","디스플레이 모드","显示方式"],
    'bgFit.cover': ["填滿裁切","Fill and crop","塗りつぶしてトリミング","채워서 자르기","填满裁切"],
    'bgFit.contain': ["完整顯示","Show full","全文表示","전체 보기","完整显示"],
    'bgFit.fill': ["拉伸鋪滿","Stretch to fill","引き伸ばして全面","늘려서 채우기","拉伸铺满"]
  };
