...
(function (root) {
  ...
  const STEPS = [
    ...
    },
    {
      id: 'playlist',
      view: 'karaoke',
      target: '.playlist-head-actions',
      mobileTarget: '.playlist-head-actions',
  ...
  const ADVANCED_OBS_STEPS = [
    ...
    },
    {
      id: 'obs-websocket',
      view: 'general',
      target: '#obs-ws-connect',
      mobileTarget: '#obs-ws-connect',
      title: 'tour.advanced.step.obsWebSocket.title',
      body: 'tour.advanced.step.obsWebSocket.body',
      hint: 'tour.advanced.step.obsWebSocket.hint',
    },
    {
      id: 'obs-create',
      view: 'general',
      target: '#obs-ws-create',
      mobileTarget: '#obs-ws-create',
      title: 'tour.advanced.step.obsCreate.title',
      body: 'tour.advanced.step.obsCreate.body',
      hint: 'tour.advanced.step.obsCreate.hint',
    },
    {
      id: 'obs-ai-separation',
      view: 'karaoke',
      prepTab: 'ai',
      target: '#home-ai-make-all',
      mobileTarget: '#home-ai-make-all',
      title: 'tour.advanced.step.obsAiSeparation.title',
      body: 'tour.advanced.step.obsAiSeparation.body',
      hint: 'tour.advanced.step.obsAiSeparation.hint',
    },
    {
      id: 'obs-dual-audio',
      view: 'general',
      target: '#dual-audio-card',
      mobileTarget: '#dual-audio-card',
  ...
  const ADVANCED_LIVE_STEPS = [
    {
      id: 'live-setlist',
      view: 'setlist',
      target: '#copy-setlist-url',
      mobileTarget: '#copy-setlist-url',
      title: 'tour.advanced.step.liveSetlist.title',
      body: 'tour.advanced.step.liveSetlist.body',
      hint: 'tour.advanced.step.liveSetlist.hint',
    },
    {
      id: 'live-delete-played',
      view: 'karaoke',
      target: '#playlist-filter-mode',
      mobileTarget: '#playlist-filter-mode',
      title: 'tour.advanced.step.liveDeletePlayed.title',
      body: 'tour.advanced.step.liveDeletePlayed.body',
      hint: 'tour.advanced.step.liveDeletePlayed.hint',
    },
    {
      id: 'live-session',
      view: 'karaoke',
    ...
    },
    {
      id: 'live-twitch',
      view: 'general',
      target: '#twitch-connect',
      mobileTarget: '#twitch-connect',
      title: 'tour.advanced.step.liveTwitch.title',
      body: 'tour.advanced.step.liveTwitch.body',
      hint: 'tour.advanced.step.liveTwitch.hint',
    },
  ];

  const ADVANCED_STEPS = [...ADVANCED_LYRICS_STEPS, ...ADVANCED_OBS_STEPS, ...ADVANCED_LIVE_STEPS];

  const dom = {};
  let basicState = loadState();
  let advancedLyricsState = loadAdvancedLyricsState();
  let advancedObsState = loadAdvancedObsState();
  let advancedLiveState = loadAdvancedLiveState();
  ...
  function loadAdvancedLiveState() {
    return loadStoredState(ADVANCED_LIVE_STORAGE_KEY, ADVANCED_TOUR_VERSION, ADVANCED_LIVE_STEPS);
  }
  ...
  const ADVANCED_KINDS = {
    lyrics: { steps: ADVANCED_LYRICS_STEPS, storageKey: ADVANCED_LYRICS_STORAGE_KEY },
    obs: { steps: ADVANCED_OBS_STEPS, storageKey: ADVANCED_OBS_STORAGE_KEY },
    live: { steps: ADVANCED_LIVE_STEPS, storageKey: ADVANCED_LIVE_STORAGE_KEY },
  };
  ...
  function saveState() {
    ...
    else if (tourKind === 'live') advancedLiveState = state;
    else basicState = state;
    const key = ADVANCED_KINDS[tourKind]?.storageKey || STORAGE_KEY;
    try { root.localStorage.setItem(key, JSON.stringify(state)); } catch (_) { /* device-local best effort */ }
  }

  function selectTour(kind) {
    tourKind = ADVANCED_KINDS[kind] ? kind : 'basic';
    state = tourKind === 'lyrics' ? advancedLyricsState
      : tourKind === 'obs' ? advancedObsState
        : tourKind === 'live' ? advancedLiveState : basicState;
  }

  function currentSteps() {
    return ADVANCED_KINDS[tourKind]?.steps || STEPS;
  }
  ...
  function cacheDom() {
    ...
    dom.viewportSkip = document.getElementById('tour-viewport-skip');
  ...
  function init() {
    ...
    dom.viewportSkip?.addEventListener('click', next);
  ...
  // Finishing or postponing the old blocking guide (elite-guide-*) must NOT
  // suppress this interactive tour: it is a new feature those users have
  // never seen, so upgraders are treated as first-time users and still get
  // the welcome prompt at least once. Kept only as a no-op so the legacy
  // keys/options stay documented at the call site.
  ...
  function isSafeToInterrupt() {
    ...
    // :not([hidden]) only checks the element itself; dormant modal templates
    // are usually hidden via an ancestor wrapper instead, so this must match
    // findBlockingModal()'s actual-visibility check or it always sees "blocked".
    const blocking = Array.from(document.querySelectorAll('[aria-modal="true"]'))
  ...
  function startAdvanced(options = {}) {
    const kind = ADVANCED_KINDS[options.kind] ? options.kind : 'lyrics';
  ...
  function next() {
    if (renderPending || (dom.next.disabled && !viewportBlocked)) return;
    const steps = currentSteps();
    ...
    state.currentStep += 1;
    saveState();
    // A placement failure is per-step (it depends on that step's target
    // size), not a property of the viewport itself — moving to a different
    // step must always get a fresh attempt instead of staying frozen behind
    // the previous step's "resize your window" warning.
    if (viewportBlocked && viewportBlockReason === 'placement') hideViewportWarning();
    renderStep({ retryViewport: true });
  }

  function previous() {
    if (renderPending || (state.currentStep <= 0 && !viewportBlocked)) return;
    dom.next.disabled = true;
  ...
  function updateCompletionContent() {
    const prefix = tourKind === 'lyrics'
      ...
      : tourKind === 'obs' ? 'tour.advanced.obsComplete'
        : tourKind === 'live' ? 'tour.advanced.liveComplete' : 'tour.complete';
    ...
    // Only the lyrics chapter chains into another chapter (OBS); OBS and Live
    // are both terminal, so they have no "next chapter" button to show.
    dom.completeAdvanced.hidden = tourKind === 'obs' || tourKind === 'live';
  ...
  function updateViewportWarning() {
    ...
    if (dom.viewportSkip) {
      // A placement failure ('placement') is specific to this step's target;
      // an actually-too-small window ('size') blocks every step's card, so
      // skipping ahead would not help and the button stays hidden.
      const steps = currentSteps();
      dom.viewportSkip.hidden = viewportBlockReason !== 'placement' || state.currentStep >= steps.length - 1;
    }
  }
  ...
  function debugStart(step = 0, options = {}) {
    const kind = ADVANCED_KINDS[options.kind] ? options.kind : 'basic';
    selectTour(kind);
  ...
  root.OnboardingTour = {
    ...
    getAdvancedState() {
      return { lyrics: { ...advancedLyricsState }, obs: { ...advancedObsState }, live: { ...advancedLiveState } };
    },
    debugStart,
    debugGoTo(step, options = {}) {
      const kind = ADVANCED_KINDS[options.kind] ? options.kind : tourKind;
      selectTour(kind);
      state.currentStep = clampStep(step);
      saveState();
      if (viewportBlocked && viewportBlockReason === 'placement') hideViewportWarning();
      if (!active) start({ resume: true, kind });
      else renderStep({ retryViewport: true });
    },
  };
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
      ...
      ADVANCED_LIVE_STEPS: ADVANCED_LIVE_STEPS.map((step) => ({ ...step })),
