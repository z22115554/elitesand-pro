...
const TwitchReplySettings = (() => {
  ...
  const REPLY_DEFINITIONS = Object.freeze([
    ...
    { key: 'duplicatePending', group: 'restrictions', label: '重複點同一首歌', defaultEnabled: true, defaultTemplate: '這首歌已在目前的重複檢查範圍內，不需重複點歌。' },
    { key: 'blockedRequest', group: 'restrictions', label: '命中黑名單規則', defaultEnabled: true, defaultTemplate: '這次點歌不符合目前規則：{reason}' },
    { key: 'streamOffline', group: 'restrictions', label: '尚未開台', defaultEnabled: true, defaultTemplate: '目前只在直播期間接受點歌，開台後再試一次。' },
    { key: 'sessionUserLimit', group: 'restrictions', label: '每人每場已達上限', defaultEnabled: true, defaultTemplate: '你本場已點滿 {limit} 首，請把機會留給其他觀眾。' },
    { key: 'sessionLimit', group: 'restrictions', label: '全場點歌已達上限', defaultEnabled: true, defaultTemplate: '本場點歌已達 {limit} 首，暫時不再接受新點歌。' },
    { key: 'consecutiveRequesterWarning', group: 'restrictions', label: '同一人連續排歌提醒', defaultEnabled: true, defaultTemplate: '{user} 已連續點歌，建議先讓其他觀眾排一首；主播仍會決定最後順序。' },
