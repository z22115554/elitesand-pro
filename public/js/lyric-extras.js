...
(function () {
  ...
  function syncTemplateButtons() {
    ...
    const supportsOffsetFineTune = (isClassic || ui.positionMode === 'stage' || isKtv) && settings.lyricPosition !== 'split';
    if (fineWrap) fineWrap.hidden = !supportsOffsetFineTune;
    if (offsetXRow) offsetXRow.hidden = !supportsOffsetFineTune || isKtv;
    if (offsetYRow) offsetYRow.hidden = !supportsOffsetFineTune;
    if (offsetXInput && supportsOffsetFineTune && !isKtv) {
