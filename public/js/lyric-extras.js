...
(function () {
  ...
  const TEMPLATE_UI = {
    ...
    jizura: { label: '文字PV', description: '日系文字 PV 風格：每句自動挑版面、進場、退場與小裝飾，整首歌預先規劃好。分析過段落的歌會依段落調整：主歌安靜、蓄力段加強、副歌最熱鬧（沒分析過的歌整首用中等強度）。為了不擋到主播，只用不會大面積蓋住畫面的版面，預設偏右（裝飾仍鋪滿全畫面，只把歌詞盡量往右推、不超出畫面）。演出引擎：JIZURA（hakoniwa，MIT 授權）。實驗性。', scope: '可調：版位（偏右／偏左／全畫面）、動作節奏（流暢／作畫感）、文字色、強調色、動畫強度。字型與字級由引擎依版面決定，不吃一般字型設定。此模板不支援拼音／諧音／翻譯；需要雙語請選「經典疊層」。', positionMode: 'fixed', supportsIntensity: true, supportsClassicControls: false },
