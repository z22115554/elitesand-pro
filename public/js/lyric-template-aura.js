...
(function () {
  ...
  LyricTemplates.register({
    ...
    onSettings(settings, ctx) {
      ...
      // 字級等視覺設定即時生效：重建目前這一行，理由同 Pulse/Facet。
      // Aura 的位置/透明度全靠 frame() 逐幀算 envelope，buildLine 本身不會設初始位置——
      // 暫停時沒有後續 onFrame 幫忙補上，重建後的行會停在瀏覽器預設位置（擠在左上角）。
      // 跟 onSeek 一樣，重建後手動呼叫一次 frame(timeMs, ctx) 落地到正確位置。
      // frame() 的 fast 分支才會讓詞「一步到位」；真正的 isFastMode() 只在拖曳進度條時
      // 為 true，這裡是設定變更、不是拖曳，若用真實 ctx 會被判定成一般播放中新詞，
      // 從「尚未唱到」的初始灰階狀態開始，用指數平滑慢慢淡回應有的顏色/透明度——
      // 暫停時後面沒有更多 frame() 補完這段過渡，畫面就永遠卡在半途的灰階（曾經真的看到過的 bug）。
      // 這裡強制 fast，讓它直接落到目前時間點該有的最終樣子，不需要過渡。
      if (current && current.lineIndex >= 0 && ctx && typeof ctx.getLyrics === 'function') {
        const idx = current.lineIndex;
        const timeMs = ctx.getCurrentTimeMs ? ctx.getCurrentTimeMs() : 0;
        retireCurrentLine(true);
        buildLine(ctx, idx);
        frame(timeMs, { ...ctx, isFastMode: () => true });
      }
    },
