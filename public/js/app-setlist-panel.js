...
(function () {
  ...
  function syncSetlistSizingHint() {
    ...
    if (SETLIST_SECTIONED_LAYOUTS.includes(layout)) {
      hint.textContent = translateText('大小與顯示首數跟著你在 OBS 拉的 Browser Source 寬高變化。');
    } else if (SETLIST_FILL_LAYOUTS.includes(layout)) {
      hint.textContent = translateText('版位固定，來源寬高只決定寬度與字級；不顯示已唱與未唱清單。');
    } else {
      hint.textContent = translateText('全幅 16:9 舞台，建議把 Browser Source 設成相同比例。');
    }
  }
