...
>
> Download the full `v0.7.6` Portable build instead, close Elitesand Pro, and unzip it fresh (keep a copy of your old folder first). Anyone already on `v0.7.3-p0-hotfix.1` or `v0.7.4-p0-test.2` can use the safe online updater directly.
...
- **Six lyric animation templates**: Classic Overlay, Stardust Flow, Prism Steps, Diagonal Confession, Tidal Mindscape, Neon Duet — each template **remembers its own** font/color/position settings independently, and named presets can be saved and applied with one click
...
### Control & Safety
- **Phone remote**: play/skip/transpose/choose-lyrics, and can switch all six templates, position, motion intensity, and saved presets in sync with the desktop panel
...
Produces `dist/update.zip` and a `dist/update.zip.sha256` containing only 64 hex characters. `-SkipBaselineCheck` is for local testing only and should never be used for a real release.
...
Produces `Elitesand-Pro-v<version>-portable.zip` in `dist/`, bundling the Node runtime and all dependencies. The current public build ships with yt-dlp and FFmpeg/ffprobe by default, and includes an exact FFmpeg source snapshot, the GPLv3 license, and build info under `licenses/ffmpeg/`; to build a version without FFmpeg, run `tools/build-portable.ps1 -WithoutFfmpeg`. After unzipping, double-click `Start Elitesand Pro.cmd` to run it — nothing else needs to be installed. The build also produces a matching `.sha256`; the launcher itself must stay plain CMD, ASCII, no BOM. It's unsigned, so a Windows SmartScreen warning is expected (More info → Run anyway).
...
>
> 請下載 `v0.7.6` 完整可攜版，關閉 Elitesand Pro 後解壓縮使用。建議先保留原資料夾副本。
